 ...
 */
import { extractCatHeredoc, tryParseBuiltinStyleTest as tryParseBuiltinStyleTestImpl, tryParseMultiLineAwkTest as tryParseMultiLineAwkTestImpl, } from "./parser-test-styles.js";
/**
 ...
 */
export function parseAwkTestFile(content, filePath) {
    ...
    while (i < lines.length) {
        ...
        if (line.includes("$awk") && line.includes(">foo")) {
            const result = tryParseBuiltinStyleTestImpl(lines, i, tryParseMultiLineAwkTestImpl);
            if (result) {
                testCases.push(result.testCase);
                i = result.nextLine;
                continue;
        ...
        if (line.match(/^\.?\/?\s*echo\s+(?:['"][^'"]*['"]|\S+)\s*>\s*foo[12]$/) &&
            i + 1 < lines.length) {
 ...
 * diff foo1 foo2 || echo 'BAD: testname'
 */
function tryParseReversedTest(lines, startLine) {
    // Extract expected output from echo line (quoted or unquoted)
    // Also matches ./echo for onetrueawk test suite
    const echoLine = lines[startLine];
    let expectedOutput = "";
    const echoQuotedMatch = echoLine.match(/^\.?\/?\s*echo\s+'([^']*)'\s*>\s*foo([12])$/);
    if (echoQuotedMatch) {
        expectedOutput = echoQuotedMatch[1];
    }
    else {
        const echoUnquotedMatch = echoLine.match(/^\.?\/?\s*echo\s+(\S+)\s*>\s*foo([12])$/);
    ...
    const programMatch = awkLines.match(/\$awk\s+(?:-[^\s]+\s+)?'([\s\S]*?)'\s*(?:\/dev\/null\s*)?>/);
    if (!programMatch) {
        return null;
    }
    ...
    for (let j = awkEndLine + 1; j < Math.min(awkEndLine + 10, lines.length); j++) {
        const line = lines[j];
        // diff/cmp line with test name
        const diffMatch = line.match(/(?:diff|cmp)\s+(?:-s\s+)?foo1\s+foo2\s*\|\|\s*\.?\/?echo\s+(['"])?(?:BAD:\s*)?([^'"]+)\1?/);
        if (diffMatch) {
        ...
        const grepMatch = line.match(/grep\s+.*\|\|\s*echo\s+(['"])?(?:BAD:\s*)?([^'"]+)\1?/);
        if (grepMatch) {
 ...
 * ' >foo1
 */
function tryParseMultiLinePipedAwkTest(lines, pipeLineIndex) {
    ...
    // Extract the awk program
    const awkLines = lines.slice(awkStartLine, awkEndLine + 1).join("\n");
    const programMatch = awkLines.match(/\$awk\s+(?:-[^\s]+\s+)?'([\s\S]*?)'\s*>/);
    if (!programMatch) {
        return null;
    }
    const program = programMatch[1];
    ...
    let testName = "";
    let nextLine = awkEndLine + 1;
    for (let j = awkEndLine + 1; j < Math.min(awkEndLine + 30, lines.length); j++) {
        const line = lines[j];
        ...
        if (line.match(/^echo\s+'[^']*$/) && !line.includes(">foo")) {
            ...
            const echoLines = [line.replace(/^echo\s+'/, "")];
            let echoEndLine = j;
 ...
 * diff foo1 foo2 || echo 'BAD: testname'
 */
function _tryParsePipedInputTest(lines, startLine) {
    const line = lines[startLine];
    // Match: echo 'input' | $awk 'program' >foo1
    // Or multi-line piped input (echo '...\n...' | $awk)
    const pipeMatch = line.match(/echo\s+(['"])([\s\S]*?)\1\s*\|\s*\$awk\s+(?:-[^\s]+\s+)?(['"])([\s\S]*?)\3\s*>\s*foo([12])/);
    if (!pipeMatch) {
    ...
    for (let j = startLine + 1; j < Math.min(startLine + 15, lines.length); j++) {
        ...
        const echoMatch = currentLine.match(/\.?\/?\s*echo\s+(['"])(.*?)\1\s*>\s*foo([12])/);
        ...
        // diff/cmp line with test name
        const diffMatch = currentLine.match(/(?:diff|cmp)\s+(?:-s\s+)?foo1\s+foo2\s*\|\|\s*\.?\/?echo\s+(['"])?(?:BAD:\s*)?([^'"]+)\1?/);
        if (diffMatch) {
            testName = cleanTestName(diffMatch[2]);
            nextLine = j + 1;
            break;
 ...
 */
function tryParseMultiLinePipedTest(lines, startLine) {
    ...
    for (let j = endLine + 1; j < Math.min(endLine + 20, lines.length); j++) {
        ...
        const echoExpMatch = currentLine.match(/^echo\s+(['"])(.*?)\1\s*>\s*foo([12])$/);
        if (echoExpMatch) {
        ...
        const echoNoQuoteMatch = currentLine.match(/^echo\s+(\S+)\s*>\s*foo([12])$/);
        ...
        if (currentLine.match(/^echo\s+["'][^"']*$/) &&
            !currentLine.includes(">foo")) {
            // This is the start of a multi-line echo - find the end
            const quoteChar = currentLine.includes('"') ? '"' : "'";
            const echoLines = [currentLine.replace(/^echo\s+["']/, "")];
            let echoEndLine = j;
            for (let k = j + 1; k < lines.length; k++) {
                if (lines[k].includes(`${quoteChar} >foo`) ||
                    lines[k].match(new RegExp(`${quoteChar}\\s*>\\s*foo`))) {
                    echoLines.push(lines[k].replace(new RegExp(`${quoteChar}\\s*>\\s*foo\\d$`), ""));
                    echoEndLine = k;
                    break;
                }
                echoLines.push(lines[k]);
 ...
 */
function tryParseHeredocTests(lines, startLine) {
 ...
 */
function tryParseTempVarStyleTest(lines, startLine) {
    ...
    const singleLineMatch = awkLine.match(/\$awk\s+(?:-[^\s]+\s+)?'([^']+)'\s*(?:([^\s>]+)\s*)?>\s*\$TEMP(\d)/);
    if (singleLineMatch) {
        ...
        const programMatch = awkLines.match(/\$awk\s+(?:-[^\s]+\s+)?'([\s\S]*?)'\s*(?:[^\s>]+\s*)?>/);
        if (!programMatch) {
            return null;
        }
    ...
    if (!inputData) {
        for (let j = startLine - 1; j >= Math.max(0, startLine - 30); j--) {
            ...
            const singleEchoMatch = line.match(/^echo\s+(['"])([\s\S]*?)\1\s*>\s*\$TEMP0$/);
            if (singleEchoMatch) {
    ...
    let testName = "";
    let nextLine = awkEndLine + 1;
    for (let j = awkEndLine + 1; j < Math.min(awkEndLine + 20, lines.length); j++) {
        const line = lines[j];
        ...
        const diffMatch = line.match(/diff\s+\$TEMP1\s+\$TEMP2\s*\|\|\s*fail\s+(['"])(?:BAD:\s*)?([^'"]+)\1/);
        if (diffMatch) {
            testName = cleanTestName(diffMatch[2]);
            nextLine = j + 1;
            break;
