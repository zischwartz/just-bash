 ...
 */
export function extractCatHeredoc(lines, startLine) {
    const line = lines[startLine];
    // Look for cat <<\EOF or cat << \EOF or cat << 'EOF' or cat <<! patterns
    const catMatch = line.match(/cat\s+<<\s*\\?(['"])?(\w+|!)\1?\s*(?:>\s*foo([12]))?/);
    if (!catMatch) {
        return null;
    }
 ...
 */
export function tryParseBuiltinStyleTest(lines, startLine, tryParseMultiLineAwkTest) {
    ...
    const awkMatch = awkLine.match(/\$awk\s+((?:-[^\s']+\s+)*)(['"])([\s\S]*?)\2(?:\s+([^\s>]+))?\)?\s*>\s*foo([12])/);
    if (!awkMatch) {
    ...
    for (let j = startLine + 1; j < Math.min(startLine + 15, lines.length); j++) {
        ...
        if (!echoMatch) {
            const echoNoQuoteMatch = line.match(/^\.?\/?echo\s+(\S+)\s*>\s*foo([12])$/);
            ...
            for (let k = j + 1; k < lines.length; k++) {
                ...
                if (kLine.match(new RegExp(`${quoteChar}\\s*>\\s*foo([12])$`))) {
                    ...
                    if (fooMatch && fooMatch[1] !== fooNum) {
                        echoLines.push(kLine.replace(new RegExp(`${quoteChar}\\s*>\\s*foo\\d$`), ""));
 ...
 */
export function tryParseMultiLineAwkTest(lines, startLine) {
    ...
    const programMatch = program.match(/\$awk\s+(?:-[^\s]+\s+)?'([\s\S]*?)'\s*(?:\/dev\/null|[^\s>]*)?/);
    if (!programMatch) {
        return null;
    }
    ...
    for (let j = endLine + 1; j < Math.min(endLine + 20, lines.length); j++) {
        ...
        const echoMatch = line.match(/^\.?\/?(echo)\s+(['"])(.*?)\2\s*>\s*foo([12])$/);
        if (echoMatch) {
        ...
        const multiLineEchoMatch = line.match(/\.?\/?echo\s+['"]([^'"]*(?:\\n[^'"]*)*)['"]\s*>\s*foo([12])/);
        ...
        const grepAndMatch = line.match(/grep\s+.*&&\s*echo\s+(['"])?(?:BAD:\s*)?([^'"]+)\1?/);
        if (grepAndMatch) {
