...
const SKIP_PATTERNS = [
    // Escape sequences in strings - test name has Unicode replacement char
    {
        ...
        reason: "NUL byte (\\x00) handling differs - we preserve NUL, OneTrue AWK truncates",
    },
    // T.misc:1>&2 tests that need specific skips (matched by program content)
    {
        pattern: /for \(i in up\) gsub\("a", "A",/,
        reason: "Test uses grep to check output - parser cannot extract expected output",
    },
    {
        pattern: /%2\$s %1\$s/,
 ...
 */
export function getSkipReason(fileName, testName, program) {
    ...
    for (const [skipKey, reason] of SKIP_TESTS) {
        if (skipKey.startsWith(`${fileName}:`) &&
