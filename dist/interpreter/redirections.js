 ...
 */
import { encodeUtf8ToBytes, latin1FromBytes, readBytesFrom, utf8ByteLength, } from "../encoding.js";
import { ControlFlowError, ErrexitError, ExecutionLimitError, ExitError, ReturnError, } from "./errors.js";
import { expandRedirectTarget, expandWord, hasQuotedMultiValueAt, } from "./expansion.js";
import { closeFd, dupFd, FIRST_USER_FD, getFdAliasMembers, getFdEntry, isFdOpen, moveFd, readFd, rememberFd, restoreFds, setFdEntry, writeFdEntry, } from "./fd-table.js";
import { checkReadonlyError } from "./helpers/readonly.js";
import { result as makeResult } from "./helpers/result.js";
import { effectiveRedirectFd, isNumericFdRedirection, } from "./numeric-fd-redirects.js";
/**
...
export const SIMPLE_REDIRECTION_POLICY = "scoped";
export const BARE_REDIRECTION_POLICY = "bare";
export const EXEC_REDIRECTION_POLICY = "persistent";
const fdLimitError = (ctx) => new ExecutionLimitError(`too many open file descriptors (max ${ctx.limits.maxFileDescriptors})`, "file_descriptors");
const hasFdCapacity = (ctx, fd) => isFdOpen(ctx, fd) ||
    (ctx.state.fileDescriptors?.size ?? 0) < ctx.limits.maxFileDescriptors;
const nextFdVariable = (ctx) => {
    let fd = Math.max(ctx.state.nextFd ?? 10, 10);
    while (isFdOpen(ctx, fd))
        fd += 1;
    return fd;
};
const parseDupTarget = (target) => {
...
const getDupSource = (ctx, sourceFd, input) => {
...
async function openOutputEntry(ctx, target, append, isClobber, handleWriteError = true) {
    ...
    if (error)
        return { error: makeResult("", error, 1) };
    if (target === "/dev/full") {
        return {
    ...
    try {
        ...
        if (!handleWriteError)
            throw error;
        return {
            error: makeResult("", `bash: ${target}: cannot open redirect target\n`, 1),
        };
    }
    return { entry: { kind: "output", path: filePath, append } };
}
async function readInputEntry(ctx, target, readwrite) {
    const filePath = ctx.fs.resolvePath(ctx.state.cwd, target);
    try {
        ...
        if (!readwrite) {
            return {
                error: makeResult("", `bash: ${target}: No such file or directory\n`, 1),
            };
        }
        try {
            await ctx.fs.writeFile(filePath, "", "binary");
...
const hereDocContent = async (ctx, hereDoc) => {
 ...
 * unsuccessful open.
 */
async function prepareRedirectionsWithState(ctx, redirections, inheritedStdin, transaction) {
    const targets = new Map();
    const dupSources = new Map();
    const standardRoute = (fd, fallback) => ctx.state.closedStandardFds?.has(fd)
        ? { kind: "closed" }
        : (getFdEntry(ctx, fd) ?? fallback);
    const standardRoutes = new Map([
    ...
    const fail = async (error, index, errorCause) => {
        const prepared = {
            ...base(),
            error: await applyRedirections(ctx, error, redirections.slice(0, index), targets, dupSources, standardRoutes),
        };
        if (errorCause) {
    ...
    const requireCapacity = async (fd, index) => {
        if (hasFdCapacity(ctx, fd))
            return null;
        const cause = fdLimitError(ctx);
        return fail(makeResult("", cause.stderr, ExecutionLimitError.EXIT_CODE), index, cause);
    };
    const persistStandard = (fd, entry) => {
    ...
    const bindTemporaryStandard = (fd, entry) => {
        standardRoutes.set(fd, entry);
        rememberFd(ctx, transaction.standardSnapshot, fd);
        if (!transaction.standardClosedSnapshot.has(fd)) {
            transaction.standardClosedSnapshot.set(fd, ctx.state.closedStandardFds?.has(fd) === true);
        }
        ctx.state.closedStandardFds?.delete(fd);
        setFdEntry(ctx, fd, entry);
    };
    const getPreparedDupSource = (sourceFd, input) => {
        if (sourceFd < FIRST_USER_FD) {
    ...
    for (let index = 0; index < redirections.length; index++) {
            ...
            if (redir.fdVariable) {
                try {
                    ...
                    if (!(error instanceof ExitError))
                        throw error;
                    return fail(makeResult(error.stdout, error.stderr, error.exitCode), index, error);
                }
                ...
                if (!transaction.fdVariableEnv.has(redir.fdVariable)) {
                    transaction.fdVariableEnv.set(redir.fdVariable, ctx.state.env.get(redir.fdVariable));
                }
                ...
                const entry = { kind: "input", content };
                if (transaction.policy === "persistent") {
        ...
        if (isDup) {
            ...
            const expanded = await expandRedirectTarget(ctx, redir.target);
        ...
        if (target.includes("\0")) {
            return fail(makeResult("", `bash: ${target.replace(/\0/g, "")}: No such file or directory\n`, 1), index);
        }
        if (redir.fdVariable) {
            try {
                checkReadonlyError(ctx, redir.fdVariable);
            ...
            if (isDup && target === "-") {
                const existingFd = Number.parseInt(ctx.state.env.get(redir.fdVariable) ?? "", 10);
            ...
            if (plannedDuplicate) {
                const source = getPreparedDupSource(plannedDuplicate.sourceFd, redir.operator === "<&");
                if (!source) {
                    return fail(makeResult("", `bash: ${plannedDuplicate.sourceFd}: Bad file descriptor\n`, 1), index);
                }
            }
            const netNeutralMove = plannedDuplicate?.move === true &&
            ...
            if (isDup) {
                ...
                if (!parsed) {
                    if (redir.operator === "<&") {
                        return fail(makeResult("", `bash: ${target}: ambiguous redirect\n`, 1), index);
                    }
                    const opened = await openOutputEntry(ctx, target, false, false);
                    if (opened.error)
                        return fail(opened.error, index);
                    entry = opened.entry;
                }
                else {
                    const source = getPreparedDupSource(parsed.sourceFd, redir.operator === "<&");
                    if (!source) {
                        return fail(makeResult("", `bash: ${parsed.sourceFd}: Bad file descriptor\n`, 1), index);
                    }
                    duplicate = parsed;
                ...
                const opened = await openOutputEntry(ctx, target, append, redir.operator === ">|");
                if (opened.error)
                    return fail(opened.error, index);
                entry = opened.entry;
            }
                ...
                const opened = await readInputEntry(ctx, target, redir.operator === "<>");
                if (opened.error)
                    return fail(opened.error, index);
                entry = opened.entry;
            }
            if (duplicate && duplicate.sourceFd >= FIRST_USER_FD) {
                ...
                if (!duplicated) {
                    return fail(makeResult("", `bash: ${duplicate.sourceFd}: Bad file descriptor\n`, 1), index);
                }
            }
            else if (entry) {
                setFdEntry(ctx, fd, entry);
        ...
        if (isNumericFdRedirection(redir)) {
            ...
            if (isDup) {
                ...
                if (!parsed) {
                    ...
                    if (opened.error)
                        return fail(opened.error, index);
                    setFdEntry(ctx, fd, opened.entry);
                    continue;
                }
                const source = getPreparedDupSource(parsed.sourceFd, redir.operator === "<&");
                if (!source) {
            ...
            if (redir.operator === ">" ||
                redir.operator === ">|" ||
                redir.operator === ">>") {
                const opened = await openOutputEntry(ctx, target, redir.operator === ">>", redir.operator === ">|");
                if (opened.error)
                    return fail(opened.error, index);
                setFdEntry(ctx, fd, opened.entry);
            }
        ...
        if (isDup) {
            ...
            if (source.kind === "entry" &&
                ...
                standardRoutes.set(effectiveFd, getFdEntry(ctx, effectiveFd));
            ...
            if (redir.operator === "<&" && effectiveFd === 0) {
                ...
                else if (source.entry.kind === "input" ||
                    source.entry.kind === "readwrite") {
                    const readable = parsed.sourceFd < FIRST_USER_FD && !isFdOpen(ctx, parsed.sourceFd)
                        ? {
                            content: source.entry.kind === "input"
        ...
        if (redir.operator === ">" ||
            ...
            redir.operator === "&>" ||
            redir.operator === "&>>") {
            const opened = await openOutputEntry(ctx, target, redir.operator === ">>" || redir.operator === "&>>", redir.operator === ">|", false);
            if (opened.error)
                return fail(opened.error, index);
            const entry = opened.entry;
        ...
        if (redir.operator === "<<<") {
            ...
            const filePath = ctx.fs.resolvePath(ctx.state.cwd, target);
            try {
                stdin = (await readBytesFrom(ctx.fs, filePath));
                stdinSourceFd = -1;
                persistStandard(effectiveFd, { kind: "input", content: stdin });
            }
            catch {
                return fail(makeResult("", `bash: ${target}: No such file or directory\n`, 1), index);
            }
        }
        else if (redir.operator === "<>") {
...
export function createRedirectionTransaction(ctx, redirections, policy) {
    const state = {
    ...
    let finished = false;
    return {
        prepare: (inheritedStdin = "") => prepareRedirectionsWithState(ctx, redirections, inheritedStdin, state),
        finish: () => {
            if (finished)
                return;
            finished = true;
...
export function preparedRedirectionError(prepared) {
    if (!prepared.error)
        throw new Error("Expected a redirection error");
...
export async function routeControlFlowError(ctx, error, redirections, prepared) {
    const exitCode = error instanceof ExitError ||
        ...
        : error instanceof ExecutionLimitError
            ...
            : 0;
    const routed = await applyRedirections(ctx, makeResult(error.stdout, error.stderr, exitCode), redirections, prepared.targets, prepared.dupSources, prepared.standardRoutes);
...
export async function withPreparedRedirections(ctx, redirections, inheritedStdin, run) {
    const transaction = createRedirectionTransaction(ctx, redirections, SIMPLE_REDIRECTION_POLICY);
    let prepared;
    try {
        prepared = await transaction.prepare(inheritedStdin);
        if (prepared.error)
            return preparedRedirectionError(prepared);
        ...
        try {
            ...
            return await applyRedirections(ctx, result, redirections, prepared.targets, prepared.dupSources, prepared.standardRoutes);
        }
        finally {
            if (prepared.stdin !== undefined) {
...
export async function applyRedirections(ctx, result, redirections, targets, dupSources = new Map(), standardRoutes = new Map(), writeErrorCommand = "bash", omitShellPrefix = false) {
    ...
    let fd1Sink = { kind: "live-stdout" };
    let fd2Sink = { kind: "live-stderr" };
    const sinkFromDupSource = (source) => {
        if (!source)
            return null;
        if (source.kind === "entry") {
            if (source.entry.kind === "dup-out") {
                if (source.entry.sourceFd === 1)
                    return { kind: "live-stdout" };
                if (source.entry.sourceFd === 2)
                    return { kind: "live-stderr" };
            }
    ...
    if (stdout !== "" || stderr !== "") {
        ...
        if (fd2Sink.kind === "invalid-output")
            pendingStderr = "";
        const deliverToFile = async (sink, content, encoding) => {
            if (sink.append) {
        ...
        if (fd1Sink === fd2Sink &&
            ...
            if (combined !== "") {
                if (fd1Sink.kind === "file") {
                    ...
                    await writeFdEntry(ctx, fd1Sink.source.entry, fd1Sink.source.descriptors, combined, getStdoutEncoding(combined));
                }
            }
        }
        else {
            for (const [content, sink, isStdout] of [
                ...
                switch (sink.kind) {
                    ...
                    case "live-stderr":
                        ...
                        break;
                    case "file":
                        await deliverToFile(sink, content, isStdout ? getStdoutEncoding(content) : getFileEncoding(content));
                        break;
                    case "descriptor":
                        await writeFdEntry(ctx, sink.source.entry, sink.source.descriptors, content, isStdout ? getStdoutEncoding(content) : getFileEncoding(content));
                        break;
                    case "invalid-output":
                        break;
                    case "discard":
    ...
    const hasOutputRedirection = redirections.length > 0 ||
        ctx.state.fileDescriptors?.has(1) === true ||
        ctx.state.fileDescriptors?.has(2) === true;
    if (hasOutputRedirection) {
        const priorTotal = (result.internalOutputAccounting?.stdout ?? 0) +
