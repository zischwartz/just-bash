...
export function createDefenseAwareCommandContext(ctx, commandName) {
    ...
    const wrappedCtx = Object.defineProperties(Object.create(Object.getPrototypeOf(ctx)), descriptors);
    if (ctx.exec) {
        wrappedCtx.exec = wrapFunction(ctx.exec, ctx.requireDefenseContext, component, "exec");
    }
    if (ctx.origCommand) {
        wrappedCtx.origCommand = wrapFunction(ctx.origCommand, ctx.requireDefenseContext, component, "origCommand");
    }
    if (ctx.fetch) {
