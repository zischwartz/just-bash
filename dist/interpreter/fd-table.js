...
function parseDupSource(raw, prefix) {
 ...
 */
export function decodeFdEntry(raw) {
...
function markContent(ctx, fd, isInput) {
    if (isInput) {
...
function leaveAliasGroup(ctx, fd) {
    ...
    if (group.size < 2) {
        for (const member of group)
            ctx.state.fdAliases?.delete(member);
    }
}
/** Put `fd` into `sourceFd`'s alias group, creating the group if needed. */
function joinAliasGroup(ctx, fd, sourceFd) {
    if (fd === sourceFd)
        return;
 ...
 */
export function setRawFd(ctx, fd, raw, isInput = false) {
    leaveAliasGroup(ctx, fd);
    writeRawFd(ctx, fd, raw, isInput);
}
export function setFdEntry(ctx, fd, entry) {
    setRawFd(ctx, fd, encodeFdEntry(entry), entry.kind === "input");
}
export function closeFd(ctx, fd) {
 ...
 */
export function dupFd(ctx, fd, sourceFd) {
...
export function moveFd(ctx, fd, sourceFd) {
    const raw = getRawFd(ctx, sourceFd);
    if (raw === undefined)
        return false;
    if (fd === sourceFd)
 ...
 */
export function advanceFd(ctx, fd, count) {
    const entry = getFdEntry(ctx, fd);
    if (entry === undefined)
        return;
    let advanced;
...
export async function writeFdEntry(ctx, entry, descriptors, content, encoding) {
    ...
    const liveEntry = descriptors
        ...
        .find((candidate) => candidate?.kind === "readwrite");
    const writeEntry = liveEntry ?? entry;
    if (writeEntry.kind !== "readwrite")
        return false;
    const updatedContent = writeEntry.content.slice(0, writeEntry.position) +
        content +
 ...
 */
export function rememberFd(ctx, snapshot, fd) {
    if (snapshot.has(fd))
        return;
    const group = aliasGroup(ctx, fd);
    snapshot.set(fd, {
 ...
 */
export function snapshotFds(ctx, fds) {
 ...
 */
export function restoreFds(ctx, snapshot) {
