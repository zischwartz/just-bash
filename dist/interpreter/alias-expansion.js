...
function appendBounded(current, fragment, maxBytes, usedBytes) {
    const fragmentBytes = utf8ByteLength(fragment);
    if (fragmentBytes > maxBytes - usedBytes.value) {
        throw new ExecutionLimitError(`alias expansion exceeds string length limit (${maxBytes} bytes)`, "string_length");
    }
...
function expandAliasOnce(ctx, node, aliasExpansionStack, expandTrailingAliases = true) {
    ...
    try {
        ...
        let fullCommand = appendBounded("", aliasValue, ctx.limits.maxStringLength, fullCommandBytes);
        ...
        if (!expandNext) {
            ...
            for (const arg of node.args) {
                ...
                fullCommand = appendBounded(fullCommand, ` ${argLiteral}`, ctx.limits.maxStringLength, fullCommandBytes);
            }
        }
        ...
        if (expandNext && node.args.length > 0) {
            ...
            while (expandTrailingAliases && newNode.args.length > 0) {
                ...
                if (expansions >= ctx.limits.maxCallDepth) {
                    throw new ExecutionLimitError(`alias expansion depth limit exceeded (${ctx.limits.maxCallDepth})`, "recursion");
                }
                expansions++;
                const tempNode = {
                    type: "SimpleCommand",
                ...
                const expandedFirst = expandAliasOnce(ctx, tempNode, aliasExpansionStack, false);
 ...
 */
function handleComplexAlias(ctx, node, aliasValue) {
    ...
    if (fullCommandBytes.value + 2 + singleQuoteCount * 3 >
        ctx.limits.maxStringLength) {
        throw new ExecutionLimitError(`alias expansion exceeds string length limit (${ctx.limits.maxStringLength} bytes)`, "string_length");
    }
