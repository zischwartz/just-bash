...
class ProcessSubstitutionFs extends InMemoryFs {
    isLiveDescriptor;
    constructor(isLiveDescriptor) {
        super();
        this.isLiveDescriptor = isLiveDescriptor;
    }
    /** Refuse anything that is not a live descriptor's backing file. */
    assertLiveDescriptor(path, operation) {
        if (this.isLiveDescriptor(path))
            return;
        this.refuse(path, operation);
    }
    refuse(path, operation) {
        throw new Error(`EROFS: read-only file system, ${operation} '${PROC_SUB_DIR}${path}'`);
    }
 ...
 */
function mountBackingFs(ctx) {
    const backing = new ProcessSubstitutionFs((relativePath) => liveEntries(ctx).some((entry) => entry.path === `${PROC_SUB_DIR}${relativePath}`));
 ...
 */
async function writeBackingFile(ctx, path, bytes) {
    try {
        ...
        if (ctx.state.processSubstitutionFsMounted)
            throw error;
        mountBackingFs(ctx);
    }
    await ctx.fs.writeFile(path, bytes, "binary");
...
async function dropBackingFile(ctx, path) {
    try {
 ...
 */
async function runBody(ctx, body, stdin) {
    const currentDepth = ctx.substitutionDepth ?? 0;
    const maxDepth = ctx.limits.maxSubstitutionDepth;
    if (currentDepth >= maxDepth) {
        throw new ExecutionLimitError(`Process substitution nesting limit exceeded (${maxDepth})`, "substitution_depth");
    }
 ...
 */
export async function openProcessSubstitution(ctx, part) {
    const entries = liveEntries(ctx);
    if (entries.length >= MAX_LIVE) {
        throw new ExecutionLimitError(`Process substitution limit exceeded (${MAX_LIVE} concurrent)`, "file_descriptors");
    }
    let bytes = "";
    if (part.direction === "input") {
        ...
        if (bytes.length > ctx.limits.maxStringLength) {
            throw new ExecutionLimitError(`process substitution: string length limit exceeded (${ctx.limits.maxStringLength} bytes)`, "string_length");
        }
    }
 ...
 */
export function markProcessSubstitutions(ctx) {
    return ctx.state.processSubstitutions?.length ?? 0;
}
/** Nothing was drained. */
const NO_WRITER_OUTPUT = {
    stdout: "",
    stderr: "",
};
/**
 ...
 */
export async function releaseProcessSubstitutions(ctx, mark) {
    const entries = ctx.state.processSubstitutions;
    if (!entries || entries.length <= mark)
