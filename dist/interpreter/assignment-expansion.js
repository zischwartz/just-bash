...
export async function expandLocalArrayAssignment(ctx, word) {
    ...
    for (const part of word.parts) {
        ...
        if (bytes > ctx.limits.maxStringLength - fullLiteralBytes) {
            throw new ExecutionLimitError(`array assignment string limit exceeded (${ctx.limits.maxStringLength} bytes)`, "string_length");
        }
        fullLiteral += value;
    ...
    const pushElement = (value, literal = false) => {
        if (elements.length >= ctx.limits.maxArrayElements) {
            throw new ExecutionLimitError(`array assignment element limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
        }
        const bytes = utf8ByteLength(value);
        ...
        if (part.type === "Literal") {
            ...
            if (part.type === "BraceExpansion") {
                ...
                if (isKeyedElement) {
                    ...
                    appendPending(wordToLiteralString({
                        type: "Word",
                        parts: [part],
                    }));
                }
                    ...
                    for (const value of braceExpanded.values)
                        pushElement(value);
                }
            }
            else {
