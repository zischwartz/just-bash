...
function cloneCompletionSpec(spec) {
    return spec
        ? {
            ...spec,
...
function cloneCompletionSpecs(specs) {
    return specs
        ? new Map(Array.from(specs, ([name, spec]) => [
            name,
            cloneCompletionSpec(spec),
        ]))
        : undefined;
}
/**
 ...
 */
function cloneFdAliases(aliases) {
    if (!aliases)
        return undefined;
...
function cloneLocalArrayScopes(scopes) {
    return scopes?.map((scope) => new Map(Array.from(scope, ([name, array]) => [
        name,
        array
...
function cloneLocalVarStack(stack) {
    return stack
        ? new Map(Array.from(stack, ([name, entries]) => [
            name,
 ...
 */
export function beginIsolatedShellState(state) {
    const saved = {
        env: state.env,
        arrays: state.arrays,
        cwd: state.cwd,
    ...
    state.localExportedVars = state.localExportedVars?.map((vars) => new Set(vars));
    ...
    state.fullyUnsetLocals = state.fullyUnsetLocals
        ...
        : undefined;
    state.tempEnvBindings = state.tempEnvBindings?.map((bindings) => new Map(bindings));
    ...
    state.defaultCompletionSpec = cloneCompletionSpec(state.defaultCompletionSpec);
