...
async function expandBracesInPartsAsync(ctx, parts, operationCounter = { count: 0 }) {
    const chargeBraceOperation = () => {
        operationCounter.count++;
        ctx.executionScope.consumeLimited("brace_operations", 1, Math.min(MAX_BRACE_OPERATIONS, ctx.limits.maxBraceExpansionResults), "brace expansion");
    };
    const pushBraceValue = (values, value) => {
        if (values.length >= ctx.limits.maxBraceExpansionResults) {
            throw new ExecutionLimitError(`brace expansion result limit exceeded (${ctx.limits.maxBraceExpansionResults})`, "array_elements");
        }
...
export async function expandRedirectTarget(ctx, word) {
    ...
    if (hasUnquotedExpansion && !isIfsEmpty(ctx.state.env)) {
        ...
        const splitWords = splitByIfsForExpansion(value, ifsChars, ctx.limits.maxArrayElements);
