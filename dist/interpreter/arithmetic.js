...
function createArithmeticResolutionContext() {
 ...
 */
async function evaluateArithValue(ctx, value, resolution) {
    if (!value) {
        return 0;
    }
...
async function evaluateResolvedArithValue(ctx, key, value, resolution) {
 ...
 *   e=1+2; $((e + 3)) => 6
 */
async function resolveArithVariable(ctx, name, resolution) {
...
export async function evaluateArithmetic(ctx, expr, isExpansionContext = false) {
    return evaluateArithmeticInternal(ctx, expr, isExpansionContext, createArithmeticResolutionContext());
}
async function evaluateArithmeticInternal(ctx, expr, isExpansionContext, resolution) {
    const evaluate = (nestedExpr, expansion = isExpansionContext) => evaluateArithmeticInternal(ctx, nestedExpr, expansion, resolution);
    switch (expr.type) {
        case "ArithNumber":
        ...
        case "ArithArrayElement": {
            ...
            const lookupArrayValue = async (key) => {
                const arrayValue = getArrayElement(ctx, expr.array, key);
                if (arrayValue !== undefined) {
                    return await evaluateResolvedArithValue(ctx, `${expr.array}[${key}]`, arrayValue, resolution);
                }
                return 0;
            };
            ...
            if (expr.index) {
                ...
                if (arrayValue !== undefined) {
                    return evaluateResolvedArithValue(ctx, envKey, arrayValue, resolution);
                }
                // Scalar decay: s[0] returns scalar value s
                if (index === 0) {
                    const scalarValue = ctx.state.env.get(expr.array);
                    if (scalarValue !== undefined) {
                        return evaluateResolvedArithValue(ctx, expr.array, scalarValue, resolution);
                    }
                }
                // Check nounset
                if (ctx.state.options.nounset) {
                    const hasAnyElement = ctx.state.env.has(expr.array) || hasArray(ctx, expr.array);
                    if (!hasAnyElement) {
        ...
        case "ArithUnary": {
            ...
            if (expr.operator === "++" || expr.operator === "--") {
                ...
                if (expr.operand.type === "ArithArrayElement") {
                    ...
                    const current = Number.parseInt(getArrayElement(ctx, arrayName, elementKey) || "0", 10) || 0;
                    const newValue = expr.operator === "++" ? current + 1 : current - 1;
                    setArrayElement(ctx, arrayName, elementKey, String(newValue), isAssoc ? "associative" : "indexed");
                    return expr.prefix ? newValue : current;
                }
                if (expr.operand.type === "ArithConcat") {
                    ...
                    for (const part of expr.operand.parts) {
                        varName += await evalConcatPartToStringAsync(ctx, part, isExpansionContext, resolution);
                    }
                ...
                if (expr.operand.type === "ArithDynamicElement") {
                    ...
                    if (varName && /^[a-zA-Z_][a-zA-Z0-9_]*$/.test(varName)) {
                        ...
                        const current = Number.parseInt(getArrayElement(ctx, varName, index) || "0", 10) || 0;
                        const newValue = expr.operator === "++" ? current + 1 : current - 1;
        ...
        case "ArithAssignment": {
            ...
            const currentValue = arrayKey === undefined
            ...
            if (arrayKey === undefined)
                ...
                setArrayElement(ctx, name, arrayKey, String(newValue), ctx.state.associativeArrays?.has(name) ? "associative" : "indexed");
            }
            return newValue;
        }
        case "ArithGroup":
        ...
        case "ArithConcat": {
            ...
            for (const part of expr.parts) {
                concatenated += await evalConcatPartToStringAsync(ctx, part, isExpansionContext, resolution);
            }
 ...
 */
async function evalConcatPartToStringAsync(ctx, expr, isExpansionContext = false, resolution = createArithmeticResolutionContext()) {
    switch (expr.type) {
        case "ArithNumber":
        ...
        case "ArithSingleQuote":
            ...
            return String(await evaluateArithmeticInternal(ctx, expr, isExpansionContext, resolution));
        case "ArithVariable":
        ...
        case "ArithConcat": {
            let result = "";
            for (const part of expr.parts) {
                result += await evalConcatPartToStringAsync(ctx, part, isExpansionContext, resolution);
            }
            return result;
        }
        default:
