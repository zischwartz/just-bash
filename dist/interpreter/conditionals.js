    ...
    switch (expr.type) {
        case "CondBinary": {
            ...
            if (isStringCompareOp(expr.operator)) {
                ...
                return compareStrings(expr.operator, left, right, !isRhsQuoted, nocasematch, true, // Always enable extglob in [[ ]] pattern matching
...
async function evaluateTestNot(ctx, args, pos) {
    const depth = ctx.executionScope.enterDepth("test_expression", ctx.limits.maxCallDepth, "test expression");
    try {
        if (args[pos] === "!") {
...
export function matchPattern(value, pattern, nocasematch = false, extglob = false, maxDepth = 256) {
    if (extglob)
        assertExtglobDepth(pattern, maxDepth);
...
function assertExtglobDepth(pattern, maximum) {
    let depth = 0;
    const groups = [];
    for (let i = 0; i < pattern.length; i++) {
        ...
        if (pattern[i] === "(") {
            ...
            if (isExtglob)
                depth++;
            if (depth > maximum) {
                throw new ExecutionLimitError(`extglob maximum depth (${maximum}) exceeded`, "recursion");
            }
        }
