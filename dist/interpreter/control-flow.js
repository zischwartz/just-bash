...
import { BreakError, ContinueError, ErrexitError, ExecutionLimitError, ExitError, GlobError, isScopeExitError, SubshellExitError, } from "./errors.js";
 ...
 */
function resolveLoopStdin(ownStdin, pipelineStdin) {
...
class CompoundOutput {
    ...
    append(stdout, stderr) {
        ...
        if (addedBytes > this.ctx.limits.maxOutputSize - this.totalBytes) {
            throwExecutionLimit(`total output size exceeded (>${this.ctx.limits.maxOutputSize} bytes), increase executionLimits.maxOutputSize`, "output_size");
        }
        if (stdout)
    ...
    appendUnaccounted(stdout, stderr) {
...
async function executeBoundedStatements(ctx, statements, output) {
    let exitCode = 0;
    try {
...
export async function executeIf(ctx, node) {
    return withPreparedRedirections(ctx, node.redirections, "", () => executeIfBody(ctx, node));
}
async function executeIfBody(ctx, node) {
...
export async function executeFor(ctx, node) {
    return withPreparedRedirections(ctx, node.redirections, "", () => executeForBody(ctx, node));
}
async function executeForBody(ctx, node) {
    const output = new CompoundOutput(ctx);
    let exitCode = 0;
    let iterations = 0;
    ...
    if (node.words === null) {
        ...
        try {
            for (const word of node.words) {
                ...
                appendBoundedElements(words, expanded.values, ctx.limits.maxArrayElements, "for-loop expansion");
            }
        }
        catch (e) {
    ...
    try {
        for (const value of words) {
            ...
            if (iterations > ctx.limits.maxLoopIterations) {
                throwExecutionLimit(`for loop: too many iterations (${ctx.limits.maxLoopIterations}), increase executionLimits.maxLoopIterations`, "iterations", output.stdout, output.stderr);
            }
            ...
            try {
                ...
                const loopResult = handleLoopError(error, output.stdout, output.stderr, ctx.state.loopDepth);
...
export async function executeCStyleFor(ctx, node) {
    return withPreparedRedirections(ctx, node.redirections, "", () => executeCStyleForBody(ctx, node));
}
async function executeCStyleForBody(ctx, node) {
...
export async function executeWhile(ctx, node, stdin = "") {
    return withPreparedRedirections(ctx, node.redirections, stdin, (prepared) => executeWhileBody(ctx, node, stdin, prepared));
}
async function executeWhileBody(ctx, node, stdin, prepared) {
    const output = new CompoundOutput(ctx);
    let exitCode = 0;
    let iterations = 0;
    ...
    try {
        while (true) {
            iterations++;
            if (iterations > ctx.limits.maxLoopIterations) {
                throwExecutionLimit(`while loop: too many iterations (${ctx.limits.maxLoopIterations}), increase executionLimits.maxLoopIterations`, "iterations", output.stdout, output.stderr);
            }
...
export async function executeUntil(ctx, node, stdin = "") {
    return withPreparedRedirections(ctx, node.redirections, stdin, (prepared) => executeUntilBody(ctx, node, stdin, prepared));
}
async function executeUntilBody(ctx, node, stdin, prepared) {
    const output = new CompoundOutput(ctx);
    let exitCode = 0;
    let iterations = 0;
    // Install groupStdin only for a stream this loop owns (see resolveLoopStdin)
    ...
    try {
        while (true) {
            iterations++;
            if (iterations > ctx.limits.maxLoopIterations) {
                throwExecutionLimit(`until loop: too many iterations (${ctx.limits.maxLoopIterations}), increase executionLimits.maxLoopIterations`, "iterations", output.stdout, output.stderr);
            }
            // Condition evaluation should not trigger errexit
...
export async function executeCase(ctx, node) {
    return withPreparedRedirections(ctx, node.redirections, "", () => executeCaseBody(ctx, node));
}
async function executeCaseBody(ctx, node) {
    const output = new CompoundOutput(ctx);
    let exitCode = 0;
    ...
    for (let i = 0; i < node.items.length; i++) {
        ...
        if (!fallThrough) {
            ...
            for (const pattern of item.patterns) {
                ...
                if (matchPattern(value, patternStr, nocasematch, extglob, ctx.limits.maxCallDepth)) {
                    matched = true;
                    break;
                }
            }
