...
const DEFAULT_IFS = " \t\n";
function assertIfsPushAllowed(length, maxArrayElements) {
    if (length >= maxArrayElements) {
        throw new ExecutionLimitError(`array element limit exceeded (${maxArrayElements})`, "array_elements");
    }
}
function pushReadField(words, wordStarts, word, start, maxArrayElements) {
 ...
 */
export function splitByIfsForRead(value, ifs, maxArrayElements, maxSplit, raw) {
    ...
    while (pos < value.length) {
        ...
        pushReadField(words, wordStarts, value.substring(wordStart, pos), wordStart, maxArrayElements);
        if (pos >= value.length) {
            break;
        }
 ...
 */
export function splitByIfsForExpansionEx(value, ifs, maxArrayElements) {
    // Empty IFS means no splitting
    if (ifs === "") {
        if (value)
            assertIfsPushAllowed(0, maxArrayElements);
...
export function splitByIfsForExpansion(value, ifs, maxArrayElements) {
