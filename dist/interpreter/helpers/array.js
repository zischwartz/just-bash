...
export function cloneArrays(arrays) {
    return new Map(Array.from(arrays ?? [], ([name, array]) => [name, cloneArray(array)]));
}
export function getArray(ctx, arrayName) {
    return ctx.state.arrays?.get(arrayName);
}
export function ensureArray(ctx, arrayName, kind = "indexed") {
...
export function setArrayKind(ctx, arrayName, kind) {
    const array = ensureArray(ctx, arrayName, kind);
    array.kind = kind;
    return array;
}
export function hasArray(ctx, arrayName) {
    return ctx.state.arrays?.has(arrayName) ?? false;
}
export function getArrayElement(ctx, arrayName, key) {
    return getArray(ctx, arrayName)?.elements.get(String(key));
}
export function hasArrayElement(ctx, arrayName, key) {
    return getArray(ctx, arrayName)?.elements.has(String(key)) ?? false;
}
export function setArrayElement(ctx, arrayName, key, value, kind) {
    const array = ensureArray(ctx, arrayName, kind);
    const normalizedKey = String(key);
    if (!array.elements.has(normalizedKey) &&
    ...
    if (utf8ByteLength(value) > ctx.limits.maxStringLength) {
        throw new ExecutionLimitError(`array value string limit exceeded (${ctx.limits.maxStringLength} bytes)`, "string_length");
    }
    array.elements.set(normalizedKey, value);
}
/** Prove a batch of distinct keys fits before any persistent mutation. */
export function assertArrayKeysFit(ctx, arrayName, keys, replace = false) {
    const prospective = new Set(replace ? [] : (getArray(ctx, arrayName)?.elements.keys() ?? []));
...
export function deleteArrayElement(ctx, arrayName, key) {
    return getArray(ctx, arrayName)?.elements.delete(String(key)) ?? false;
}
export function deleteArray(ctx, arrayName) {
...
export function clearArray(ctx, arrayName) {
    ensureArray(ctx, arrayName, ctx.state.associativeArrays?.has(arrayName) ? "associative" : "indexed").elements.clear();
}
/**
