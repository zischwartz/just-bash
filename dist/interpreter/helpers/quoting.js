 ...
 */
export function quotedValueByteLength(value) {
    if (needsDollarQuoting(value))
        return dollarQuotedByteLength(value);
    if (value === "")
        return 2;
 ...
 */
export function quotedArrayValueByteLength(value) {
    if (needsDollarQuoting(value))
        return dollarQuotedByteLength(value);
    let bytes = utf8ByteLength(value) + 2;
    for (const char of value) {
...
function dollarQuotedByteLength(value) {
    let bytes = 3; // $'...'
    for (const char of value) {
