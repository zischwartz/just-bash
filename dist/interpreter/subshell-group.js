...
import { advanceFd, getFdAliasMembers, getFdEntry, } from "./fd-table.js";
...
stdinOwned = false) {
    ...
    try {
        return await withPreparedRedirections(ctx, node.redirections, stdin, (prepared) => executeSubshellBody(ctx, node, prepared.stdin ?? stdin, executeStatement, stdinOwned || prepared.stdin !== undefined));
    }
    finally {
        const consumedDescriptors = new Map();
        for (const [fd, parentEntry] of parentDescriptors) {
            const childEntry = getFdEntry(ctx, fd);
            const consumed = parentEntry.kind === "input" && childEntry?.kind === "input"
            ...
            consumedDescriptors.set(sourceFd, Math.max(consumedDescriptors.get(sourceFd) ?? 0, consumed));
        }
        restoreState();
...
async function executeSubshellBody(ctx, node, stdin, executeStatement, stdinOwned) {
 ...
 */
stdinOwned = false) {
    return withPreparedRedirections(ctx, node.redirections, stdin, (prepared) => executeGroupBody(ctx, node, stdin, executeStatement, stdinOwned, prepared));
}
async function executeGroupBody(ctx, node, stdin, executeStatement, stdinOwned, prepared) {
