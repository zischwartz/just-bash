...
function boundedAppender(maxBytes) {
    const chunks = [];
    let bytes = 0;
    return {
        append(value) {
            ...
            if (valueBytes > maxBytes - bytes) {
                throw new ExecutionLimitError(`prompt expansion exceeds string length limit (${maxBytes} bytes)`, "string_length");
            }
            if (value)
                chunks.push(value);
            bytes += valueBytes;
 ...
 */
function simpleStrftime(format, date, maxBytes) {
        ...
        if (char === "\\") {
            ...
            switch (next) {
                ...
                case "d": {
                    ...
                    result.append(`${weekdays[now.getDay()]} ${months[now.getMonth()]} ${dayStr}`);
                    i += 2;
                    break;
                }
                case "t": {
                ...
                case "D":
                    ...
                    if (i + 2 < value.length && value[i + 2] === "{") {
                        ...
                        if (closeIdx !== -1) {
                            ...
                            result.append(simpleStrftime(format, now, ctx.limits.maxStringLength));
                            i = closeIdx + 1;
                        }
                        else {
