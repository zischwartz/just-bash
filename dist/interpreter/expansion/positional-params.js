...
function getPositionalParams(ctx) {
    ...
    if (numParams > ctx.limits.maxArrayElements) {
        throw new ExecutionLimitError(`positional parameter element limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
    }
    const params = [];
    let bytes = 0;
    for (let i = 1; i <= numParams; i++) {
        ...
        if (valueBytes > ctx.limits.maxStringLength - bytes) {
            throw new ExecutionLimitError(`positional parameter string limit exceeded (${ctx.limits.maxStringLength} bytes)`, "string_length");
        }
        params.push(value);
        bytes += valueBytes;
    }
...
function appendBounded(current, fragment, maxBytes) {
    if (utf8ByteLength(fragment) > maxBytes - utf8ByteLength(current)) {
        throw new ExecutionLimitError(`positional expansion string limit exceeded (${maxBytes} bytes)`, "string_length");
    }
    return current + fragment;
}
function pushBounded(values, value, usedBytes, ctx) {
    if (values.length >= ctx.limits.maxArrayElements) {
        throw new ExecutionLimitError(`positional expansion element limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
    }
    const bytes = utf8ByteLength(value);
    if (bytes > ctx.limits.maxStringLength - usedBytes.value) {
        throw new ExecutionLimitError(`positional expansion string limit exceeded (${ctx.limits.maxStringLength} bytes)`, "string_length");
    }
    values.push(value);
...
export async function handlePositionalPatternReplacement(ctx, wordParts, expandPart, expandWordPartsAsync) {
    ...
    for (let i = 0; i < patReplAtIndex; i++) {
        prefix = appendBounded(prefix, await expandPart(ctx, dqPart.parts[i]), ctx.limits.maxStringLength);
    }
    // Expand suffix (parts after ${@/...})
    let suffix = "";
    for (let i = patReplAtIndex + 1; i < dqPart.parts.length; i++) {
        suffix = appendBounded(suffix, await expandPart(ctx, dqPart.parts[i]), ctx.limits.maxStringLength);
    }
    if (params.length === 0) {
        const combined = prefix + suffix;
        return { values: combined ? [combined] : [], quoted: true };
    ...
    try {
        ...
        for (const param of params) {
            ...
            while (referencePattern.exec(replacement) !== null)
                referenceCount++;
            const upperBound = paramBytes +
            ...
            pushBounded(replacedParams, re.replace(param, replacement), replacedBytes, ctx);
        }
    }
    catch (error) {
    ...
    if (patReplIsStar) {
        ...
        const ifsSep = getIfsSeparator(ctx.state.env);
        const joinBytes = replacedBytes.value +
    ...
    if (utf8ByteLength(prefix) + replacedBytes.value + utf8ByteLength(suffix) >
...
export async function handlePositionalPatternRemoval(ctx, wordParts, expandPart, expandWordPartsAsync) {
    ...
    for (const param of params) {
        pushBounded(strippedParams, applyPatternRemoval(ctx, param, regexStr, operation.side, operation.greedy), strippedBytes, ctx);
    }
    if (patRemIsStar) {
        // "${*#...}" - join all params with IFS into one word
        const ifsSep = getIfsSeparator(ctx.state.env);
        const joinBytes = strippedBytes.value +
    ...
    // "${@#...}" - each param is a separate word
    if (utf8ByteLength(prefix) + strippedBytes.value + utf8ByteLength(suffix) >
        ctx.limits.maxStringLength) {
        throw new ExecutionLimitError(`positional expansion string limit exceeded (${ctx.limits.maxStringLength} bytes)`, "string_length");
    }
