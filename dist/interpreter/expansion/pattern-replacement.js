...
export function applyPatternReplacementBounded(ctx, value, regex, replacement, replaceAll) {
    const output = new BoundedStringBuilder(ctx.limits.maxStringLength, "pattern replacement", () => new ExecutionLimitError(`pattern replacement: string length limit exceeded (${ctx.limits.maxStringLength} bytes)`, "string_length"));
    regex.lastIndex = 0;
    let lastIndex = 0;
    let match = regex.exec(value);
    while (match !== null) {
        ctx.executionScope.consumeLimited("pattern_operations", 1, ctx.limits.maxGlobOperations, "pattern replacement");
