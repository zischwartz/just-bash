 ...
 */
export function applyPatternRemoval(ctx, value, regexStr, side, greedy) {
    // Use 's' flag (dotall) so that . matches newlines (bash ? matches any char including newline)
    if (side === "prefix") {
        ctx.executionScope.consumeLimited("pattern-removal", Math.max(1, value.length), ctx.limits.maxGlobOperations, "pattern removal");
