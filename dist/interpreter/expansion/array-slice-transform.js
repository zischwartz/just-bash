...
export async function handleArraySlicing(ctx, wordParts, evaluateArithmetic) {
    ...
    if (elements.length > ctx.limits.maxArrayElements) {
        throw new ExecutionLimitError(`array transform element limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
    }
...
export function handleArrayTransform(ctx, wordParts) {
    ...
    if (elements.length === 0) {
        ...
        if (scalarValue !== undefined) {
            ...
            if (utf8ByteLength(resultValue) > ctx.limits.maxStringLength) {
                throw new ExecutionLimitError(`array transform string limit exceeded (${ctx.limits.maxStringLength} bytes)`, "string_length");
            }
    ...
    if (isStar) {
        ...
        const ifsSep = getIfsSeparator(ctx.state.env);
        const separatorBytes = Math.max(0, transformedValues.length - 1) * utf8ByteLength(ifsSep);
