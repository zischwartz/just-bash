...
function pushExpansionValue(ctx, values, value, bytes) {
    if (values.length >= ctx.limits.maxArrayElements) {
        throw new ExecutionLimitError(`unquoted expansion element limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
    }
    const valueBytes = utf8ByteLength(value);
    if (valueBytes > ctx.limits.maxStringLength - bytes.value) {
        throw new ExecutionLimitError(`unquoted expansion string limit exceeded (${ctx.limits.maxStringLength} bytes)`, "string_length");
    }
    values.push(value);
    bytes.value += valueBytes;
}
function joinExpansionValues(ctx, values, separator, valueBytes) {
...
function createGlobExpander(ctx) {
    return new GlobExpander(ctx.fs, ctx.state.cwd, ctx.state.env, {
        ...
        consumeOperation: () => ctx.executionScope.consumeLimited("glob_operations", 1, ctx.limits.maxGlobOperations, "glob expansion"),
    });
}
/**
...
async function applyGlobExpansion(ctx, words) {
    ...
    for (const w of words) {
        if (hasGlobPattern(w, ctx.state.shoptOptions.extglob)) {
            ...
            if (matches.length > 0) {
                appendBoundedElements(expandedValues, matches, ctx.limits.maxArrayElements, "unquoted glob expansion");
            }
...
export async function handleUnquotedArrayPatternReplacement(ctx, wordParts, expandWordPartsAsync, expandPart) {
    ...
    try {
        ...
        for (const value of values) {
            pushExpansionValue(ctx, replacedValues, applyPatternReplacementBounded(ctx, value, re, replacement, operation.all), replacedBytes);
        }
    }
    catch (error) {
    ...
    if (unquotedArrayIsStar) {
        ...
        const joined = joinExpansionValues(ctx, replacedValues, ifsSep, replacedBytes.value);
        if (ifsEmpty) {
            return { values: joined ? [joined] : [], quoted: false };
        }
        return {
            values: splitByIfsForExpansion(joined, ifsChars, ctx.limits.maxArrayElements),
            quoted: false,
        };
    }
    ...
    for (const val of replacedValues) {
        if (val === "") {
            ...
            for (const word of splitByIfsForExpansion(val, ifsChars, ctx.limits.maxArrayElements)) {
...
export async function handleUnquotedArrayPatternRemoval(ctx, wordParts, expandWordPartsAsync, expandPart) {
    ...
    for (const value of values) {
        pushExpansionValue(ctx, strippedValues, applyPatternRemoval(ctx, value, regexStr, operation.side, operation.greedy), strippedBytes);
    }
    // For unquoted, we need to IFS-split the result
    const ifsChars = getIfs(ctx.state.env);
    const ifsEmpty = isIfsEmpty(ctx.state.env);
    if (unquotedArrayIsStar) {
        // ${arr[*]#...} unquoted - join with IFS, then split
        const ifsSep = getIfsSeparator(ctx.state.env);
        const joined = joinExpansionValues(ctx, strippedValues, ifsSep, strippedBytes.value);
        if (ifsEmpty) {
            return { values: joined ? [joined] : [], quoted: false };
        }
        return {
...
export async function handleUnquotedPositionalPatternRemoval(ctx, wordParts, expandWordPartsAsync, expandPart) {
    ...
    for (const param of params) {
        pushExpansionValue(ctx, strippedParams, applyPatternRemoval(ctx, param, regexStr, operation.side, operation.greedy), strippedBytes);
    }
    // For unquoted, we need to IFS-split the result
    const ifsChars = getIfs(ctx.state.env);
    const ifsEmpty = isIfsEmpty(ctx.state.env);
    if (unquotedPosPatRemIsStar) {
        ...
        const ifsSep = getIfsSeparator(ctx.state.env);
        const joined = joinExpansionValues(ctx, strippedParams, ifsSep, strippedBytes.value);
        if (ifsEmpty) {
            return { values: joined ? [joined] : [], quoted: false };
        }
        return {
...
export async function handleUnquotedPositionalSlicing(ctx, wordParts, evaluateArithmetic, expandPart) {
    ...
    if (slicedParams.length === 0) {
        ...
        return {
            values: splitByIfsForExpansion(combined, ifsChars, ctx.limits.maxArrayElements),
            quoted: false,
        };
    }
    let allWords;
    if (unquotedSliceIsStar) {
        ...
        if (ifsEmpty) {
            ...
            allWords = splitByIfsForExpansion(joined, ifsChars, ctx.limits.maxArrayElements);
        }
    }
    else {
        ...
        if (ifsEmpty) {
            ...
            for (let i = 0; i < slicedParams.length; i++) {
                ...
                if (param === "") {
                    ...
                    const parts = splitByIfsForExpansion(param, ifsChars, ctx.limits.maxArrayElements);
                    appendBoundedElements(allWords, parts, ctx.limits.maxArrayElements, "unquoted expansion");
                }
            }
        }
    }
    ...
    if (isStar) {
        ...
        if (ifsEmpty) {
            ...
            for (const val of values) {
                ...
                const parts = splitByIfsForExpansion(val, ifsChars, ctx.limits.maxArrayElements);
                appendBoundedElements(allWords, parts, ctx.limits.maxArrayElements, "unquoted expansion");
            }
        }
        else {
    ...
    if (op.star) {
        ...
        if (ifsEmpty) {
            ...
            for (const name of matchingVars) {
                const parts = splitByIfsForExpansion(name, ifsChars, ctx.limits.maxArrayElements);
                appendBoundedElements(allWords, parts, ctx.limits.maxArrayElements, "unquoted expansion");
            }
        }
    }
    ...
    if (op.star) {
        ...
        if (ifsEmpty) {
            ...
            for (const key of keys) {
                const parts = splitByIfsForExpansion(key, ifsChars, ctx.limits.maxArrayElements);
                appendBoundedElements(allWords, parts, ctx.limits.maxArrayElements, "unquoted expansion");
            }
        }
    }
    ...
    // Each param becomes a separate word, then each is subject to IFS splitting.
    {
        ...
        if (ifsEmpty) {
            ...
            for (const word of rawWords) {
                if (word === "")
                    continue;
                const parts = splitByIfsForExpansion(word, ifsChars, ctx.limits.maxArrayElements);
                appendBoundedElements(words, parts, ctx.limits.maxArrayElements, "unquoted expansion");
            }
        }
        else {
