...
function pushSplitWord(ctx, words, value) {
    if (words.length >= ctx.limits.maxArrayElements) {
        throw new ExecutionLimitError(`word splitting element limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
    }
    words.push(value);
}
/**
...
export async function smartWordSplit(ctx, wordParts, ifsChars, _ifsPattern, expandPartFn) {
    ...
    for (const segment of segments) {
        if (!segment.isSplittable) {
            ...
            const { words: parts, hadLeadingDelimiter, hadTrailingDelimiter, } = splitByIfsForExpansionEx(segment.value, ifsChars, ctx.limits.maxArrayElements);
...
async function smartWordSplitWithUnquotedLiterals(ctx, wordParts, ifsChars, _ifsPattern, expandPartFn) {
    ...
    for (const segment of segments) {
        if (!segment.isSplittable) {
            ...
            const { words: parts, hadTrailingDelimiter } = splitByIfsForExpansionEx(segment.value, ifsChars, ctx.limits.maxArrayElements);
            if (parts.length === 0) {
                // Empty expansion produces nothing
                if (hadTrailingDelimiter) {
                    pendingWordBreak = true;
