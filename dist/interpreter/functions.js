...
import { applyRedirections, createRedirectionTransaction, preparedRedirectionError, routeControlFlowError, SIMPLE_REDIRECTION_POLICY, } from "./redirections.js";
...
stdinRedirected = false) {
    ...
    let prepared = null;
    const redirectionTransaction = createRedirectionTransaction(ctx, func.redirections, SIMPLE_REDIRECTION_POLICY);
    try {
        ...
        const stdinOwned = stdinRedirected || prepared.stdin !== undefined || stdin !== "";
        const execResult = await ctx.executeCommand(func.body, effectiveStdin, stdinOwned);
        ...
        return applyRedirections(ctx, execResult, func.redirections, prepared.targets, prepared.dupSources, prepared.standardRoutes);
    }
    catch (error) {
