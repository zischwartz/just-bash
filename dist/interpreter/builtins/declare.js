...
import { assertArrayKeysFit, clearArray, cloneArray, getArray, getArrayElement, getArrayIndices, hasArray, setArrayElement, setArrayKind, } from "../helpers/array.js";
...
export async function handleDeclare(ctx, args) {
    ...
    for (const arg of processedArgs) {
        ...
        if (arrayMatch && !removeArray) {
            ...
            const parsedAssocEntries = declareAssoc && content.includes("[")
                ? parseAssocArrayLiteral(content, arrayParseLimits)
                : undefined;
            const parsedElements = parsedAssocEntries === undefined
                ...
                : undefined;
            ctx.executionScope.consumeWork(parsedAssocEntries?.length ?? parsedElements?.length ?? 0, "declare array assignment");
            ...
            if (declareAssoc && content.includes("[")) {
                for (const [key, rawValue] of parsedAssocEntries ?? []) {
                    setArrayElement(ctx, name, key, applyAssignmentTildeExpansion(ctx, rawValue), "associative");
                }
            }
                ...
                if (hasKeyedElements) {
                    ...
                    for (const element of elements) {
                        ...
                        if (keyedMatch) {
                            ...
                            setArrayElement(ctx, name, index, applyAssignmentTildeExpansion(ctx, rawValue));
        ...
        if (arrayAppendMatch && !removeArray) {
            ...
            if (ctx.state.associativeArrays?.has(name)) {
                ...
                assertArrayKeysFit(ctx, name, entries.map(([key]) => key));
                for (const [key, rawValue] of entries) {
                    setArrayElement(ctx, name, key, rawValue, "associative");
                }
            }
            else {
                ctx.executionScope.consumeWork(newElements.length, "declare array append");
                ...
                assertArrayKeysFit(ctx, name, Array.from({ length: newElements.length }, (_, i) => startIndex + i));
                // Append new elements
        ...
        if (appendMatch) {
            ...
            if (isInteger(ctx, name)) {
                ...
                setArrayElement(ctx, name, 0, boundedJoin([existing, appendValue], "", ctx.limits.maxStringLength, "declare append"));
            }
            else {
                ...
                ctx.state.env.set(name, boundedJoin([existing, appendValue], "", ctx.limits.maxStringLength, "declare append"));
            }
            // Mark as local if inside a function
            markAsLocalIfNeeded(name);
            if (declareReadonly) {
    ...
    for (const arg of processedArgs) {
        ...
        if (arrayAppendMatch) {
            ...
            if (ctx.state.associativeArrays?.has(name)) {
                ...
                ctx.executionScope.consumeWork(newElements.length, "readonly array append");
                // For indexed arrays, get current highest index and append
                const existingIndices = getArrayIndices(ctx, name);
                // If variable was a scalar, convert it to array element 0
                let startIndex = 0;
        ...
        if (appendMatch) {
            ...
            // Append to existing value (or set if not defined)
            const existing = ctx.state.env.get(name) ?? "";
            ctx.state.env.set(name, boundedJoin([existing, appendValue], "", ctx.limits.maxStringLength, "readonly append"));
            markReadonly(ctx, name);
            continue;
        }
