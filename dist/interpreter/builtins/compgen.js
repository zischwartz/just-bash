...
import { clearArray, cloneArray, ensureArray, getArray, hasArray, } from "../helpers/array.js";
import { failure, result, success } from "../helpers/result.js";
const preserveWordlistEscapes = (part) => {
    if (part.type === "Escaped")
        return AST.literal(`\\${part.value}`);
...
export async function handleCompgen(ctx, args) {
    ...
    const remainingCompletionCapacity = () => ctx.limits.maxArrayElements - completions.length;
    const appendCompletions = (values) => {
        for (const value of values) {
            if (completions.length >= ctx.limits.maxArrayElements) {
                throw new ExecutionLimitError(`compgen: completion element limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
            }
            if (utf8ByteLength(value) > ctx.limits.maxStringLength) {
                throw new ExecutionLimitError(`compgen: completion string limit exceeded (${ctx.limits.maxStringLength} bytes)`, "string_length");
            }
    ...
    if (dirnamesOption) {
        const dirCompletions = await getDirectoryCompletions(ctx, searchPrefix, remainingCompletionCapacity());
        appendCompletions(dirCompletions);
    }
    // Handle -o default (show files)
    if (defaultOption) {
        const fileCompletions = await getFileCompletions(ctx, searchPrefix, remainingCompletionCapacity());
    ...
    for (const actionType of actionTypes) {
        if (actionType === "variable") {
            const vars = getVariableNames(ctx, searchPrefix, remainingCompletionCapacity());
            appendCompletions(vars);
        }
        else if (actionType === "export") {
            const vars = getExportedVariableNames(ctx, searchPrefix, remainingCompletionCapacity());
            appendCompletions(vars);
        }
        else if (actionType === "function") {
            const funcs = getFunctionNames(ctx, searchPrefix, remainingCompletionCapacity());
            appendCompletions(funcs);
        }
        else if (actionType === "builtin") {
            const builtins = getBuiltinNames(ctx, searchPrefix, remainingCompletionCapacity());
            appendCompletions(builtins);
        }
        else if (actionType === "keyword") {
            const keywords = getKeywordNames(ctx, searchPrefix, remainingCompletionCapacity());
            appendCompletions(keywords);
        }
        else if (actionType === "alias") {
            const aliases = getAliasNames(ctx, searchPrefix, remainingCompletionCapacity());
            appendCompletions(aliases);
        }
        else if (actionType === "shopt") {
            const shopts = getShoptNames(ctx, searchPrefix, remainingCompletionCapacity());
            appendCompletions(shopts);
        }
        else if (actionType === "helptopic") {
            const topics = getHelpTopicNames(ctx, searchPrefix, remainingCompletionCapacity());
            appendCompletions(topics);
        }
        else if (actionType === "directory") {
            const dirs = await getDirectoryCompletions(ctx, searchPrefix, remainingCompletionCapacity());
            appendCompletions(dirs);
        }
        else if (actionType === "file") {
            const files = await getFileCompletions(ctx, searchPrefix, remainingCompletionCapacity());
            appendCompletions(files);
        }
        else if (actionType === "user") {
            const users = getUserNames(ctx, searchPrefix, remainingCompletionCapacity());
            appendCompletions(users);
        }
        else if (actionType === "command") {
            const commands = await getCommandCompletions(ctx, searchPrefix, remainingCompletionCapacity());
    ...
    if (wordlist !== null) {
        try {
            ...
            const words = splitWordlist(ctx, expandedWordlist, remainingCompletionCapacity(), searchPrefix);
    ...
    if (plusdirsOption) {
        const dirCompletions = await getDirectoryCompletions(ctx, searchPrefix, remainingCompletionCapacity(), completionSet);
    ...
    if (functionName !== null) {
        ...
        if (func) {
            ...
            try {
                ...
                const compreplyValues = getCompreplyValues(ctx, remainingCompletionCapacity());
    ...
    if (excludePattern !== null) {
        ...
        filteredCompletions = completions.filter((c) => {
            ...
            const matches = matchPattern(c, pattern, false, true, ctx.limits.maxCallDepth);
    ...
    const outputBuilder = new BoundedStringBuilder(ctx.limits.maxOutputSize, "compgen");
 ...
 */
function getVariableNames(ctx, prefix, maximum) {
 ...
 */
function getExportedVariableNames(ctx, prefix, maximum) {
    const candidates = createCandidateCollector(ctx, prefix, maximum);
 ...
 */
function getFunctionNames(ctx, prefix, maximum) {
    const candidates = createCandidateCollector(ctx, prefix, maximum);
    for (const name of ctx.state.functions.keys())
        candidates.add(name);
    return candidates.build();
}
/**
 ...
 */
function getBuiltinNames(ctx, prefix, maximum) {
    const candidates = createCandidateCollector(ctx, prefix, maximum);
 ...
 */
function getKeywordNames(ctx, prefix, maximum) {
    const candidates = createCandidateCollector(ctx, prefix, maximum);
    for (const name of SHELL_KEYWORDS)
        candidates.add(name);
    return candidates.build();
}
/**
 ...
 */
function getAliasNames(ctx, prefix, maximum) {
    const candidates = createCandidateCollector(ctx, prefix, maximum);
 ...
 */
function getShoptNames(ctx, prefix, maximum) {
    const candidates = createCandidateCollector(ctx, prefix, maximum);
    for (const name of SHOPT_OPTIONS)
        candidates.add(name);
    return candidates.build();
}
/**
 ...
 */
function getHelpTopicNames(ctx, prefix, maximum) {
    const candidates = createCandidateCollector(ctx, prefix, maximum);
 ...
 */
async function getDirectoryCompletions(ctx, prefix, maximum = ctx.limits.maxArrayElements, excluded) {
 ...
 */
async function getFileCompletions(ctx, prefix, maximum = ctx.limits.maxArrayElements) {
 ...
 */
function getUserNames(ctx, prefix, maximum) {
 ...
 */
async function getCommandCompletions(ctx, prefix, maximum) {
    const candidates = createCandidateCollector(ctx, prefix, maximum);
    // Add builtins
...
function createCandidateCollector(ctx, prefix, maximum, excluded) {
...
async function expandWordlistString(ctx, wordlist) {
    ...
    return await expandWord(ctx, AST.word(wordNode.parts.map(preserveWordlistEscapes)));
}
/**
 ...
 */
function splitWordlist(ctx, wordlist, maximum, prefix) {
    const ifs = ctx.state.env.get("IFS") ?? " \t\n";
    if (ifs.length === 0) {
        if (prefix !== null && !wordlist.startsWith(prefix))
            return [];
        if (maximum === 0) {
            throw new ExecutionLimitError(`compgen: completion element limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
        }
...
function restoreCompletionArray(ctx, name, saved) {
    ctx.state.arrays ??= new Map();
    if (saved)
        ctx.state.arrays.set(name, cloneArray(saved));
    else
 ...
 */
function getCompreplyValues(ctx, maximum) {
    const values = [];
