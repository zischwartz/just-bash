...
function pushParsed(values, value, limits) {
    if (limits && values.length >= limits.maxElements) {
        throw new ExecutionLimitError(`array element limit exceeded (${limits.maxElements})`, "array_elements");
    }
    values.push(value);
}
/**
 ...
 */
export function parseArrayElements(content, limits) {
    if (limits && utf8ByteLength(content) > limits.maxStringBytes) {
        throw new ExecutionLimitError(`array assignment string limit exceeded (${limits.maxStringBytes} bytes)`, "string_length");
    }
    const elements = [];
    let current = "";
 ...
 */
export function parseAssocArrayLiteral(content, limits) {
    if (limits && utf8ByteLength(content) > limits.maxStringBytes) {
        throw new ExecutionLimitError(`array assignment string limit exceeded (${limits.maxStringBytes} bytes)`, "string_length");
    }
    const entries = [];
