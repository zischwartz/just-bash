...
import { clearArray, cloneArray, getArray, setArrayElement, } from "../helpers/array.js";
 ...
 */
export function parseAssignment(arg, limits) {
...
export async function setVariable(ctx, assignment, options = {}) {
    ...
    if (isArray && arrayElements) {
        ...
        try {
            ...
            if (priorArraySnapshot)
                ctx.state.arrays.set(name, priorArraySnapshot);
            else
                ctx.state.arrays.delete(name);
            if (priorScalar === undefined)
                ctx.state.env.delete(name);
            else
                ctx.state.env.set(name, priorScalar);
            throw error;
        }
    }
