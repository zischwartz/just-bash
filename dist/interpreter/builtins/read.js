...
export function handleRead(ctx, args, stdin, stdinSourceFd = -1) {
    ...
    const readsFromFd = fileDescriptor > 0 &&
    ...
    if (readsFromFd) {
        ...
        if ("error" in readable) {
            ...
            const writeOnly = readable.error === "write-only" ||
                fileDescriptor === 1 ||
                fileDescriptor === 2;
    ...
    const appendLine = (value) => {
        ...
        if (bytes > ctx.limits.maxStringLength - lineBytes) {
            throw new ExecutionLimitError(`read: string length limit exceeded (${ctx.limits.maxStringLength} bytes)`, "string_length");
        }
        line += value;
        lineBytes += bytes;
    };
    const assertLineLimit = (value) => {
    ...
    if (arrayName) {
        ...
        const { words } = splitByIfsForRead(line, ifs, maxArrayElements, undefined, raw);
        clearArray(ctx, arrayName);
    ...
    const { words, wordStarts } = splitByIfsForRead(line, ifs, maxArrayElements, maxSplit, raw);
