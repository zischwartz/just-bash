...
function pushBoundedLine(lines, line, usedBytes, maxBytes) {
    const nextBytes = usedBytes + utf8ByteLength(line) + 1;
    if (nextBytes > maxBytes) {
        throw new ExecutionLimitError(`shopt: output size limit exceeded (${maxBytes} bytes)`, "output_size");
    }
    lines.push(line);
    return nextBytes;
}
...
export function handleShopt(ctx, args) {
    ...
    if (args.length > maxArrayElements) {
        throw new ExecutionLimitError(`shopt: argument limit exceeded (${maxArrayElements})`, "array_elements");
    }
    const maxOutputBytes = Math.min(ctx.limits.maxOutputSize, ctx.limits.maxStringLength);
    // Parse arguments
    ...
    for (const name of optionNames) {
        if (!isShoptOption(name) && !isStubOption(name)) {
            stderrBytes = pushBoundedLine(stderrLines, `shopt: ${name}: invalid shell option name`, stderrBytes, maxOutputBytes);
            hasError = true;
            continue;
        }
        if (setFlag) {
            ...
            if (isShoptOption(name)) {
                ...
                if (quietFlag) {
                    ...
                    outputBytes = pushBoundedLine(output, `shopt ${value ? "-s" : "-u"} ${name}`, outputBytes, maxOutputBytes);
                    ...
                    outputBytes = pushBoundedLine(output, `${name}\t\t${value ? "on" : "off"}`, outputBytes, maxOutputBytes);
                ...
                if (quietFlag) {
                    ...
                    outputBytes = pushBoundedLine(output, `shopt -u ${name}`, outputBytes, maxOutputBytes);
                    hasError = true; // Stub options are always off
                }
                else {
                    outputBytes = pushBoundedLine(output, `${name}\t\toff`, outputBytes, maxOutputBytes);
                    hasError = true; // Stub options are always off
                }
            }
        }
...
function handleSetOptions(ctx, optionNames, setFlag, unsetFlag, printFlag, quietFlag) {
    ...
    for (const name of optionNames) {
        ...
        if (!isImplemented && !isNoOp) {
            stderrBytes = pushBoundedLine(stderrLines, `shopt: ${name}: invalid option name`, stderrBytes, maxOutputBytes);
            hasError = true;
            continue;
        }
        if (isNoOp) {
            ...
            if (setFlag || unsetFlag) {
                ...
                if (quietFlag) {
                    ...
                    outputBytes = pushBoundedLine(output, `set +o ${name}`, outputBytes, maxOutputBytes);
        ...
        if (setFlag) {
            ...
            if (quietFlag) {
                ...
                outputBytes = pushBoundedLine(output, `set ${value ? "-o" : "+o"} ${name}`, outputBytes, maxOutputBytes);
