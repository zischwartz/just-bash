...
import { assertArrayKeysFit, clearArray, cloneArray, getArray, getArrayIndices, setArrayElement, setArrayKind, } from "../helpers/array.js";
...
export async function handleLocal(ctx, args) {
    ...
    for (const arg of processedArgs) {
        ...
        if (appendMatch) {
            ...
            ctx.state.env.set(name, boundedJoin([existing, appendValue], "", ctx.limits.maxStringLength, "local append"));
            // Track local variable depth for bash-specific unset scoping
            markLocalVarDepth(ctx, name);
            // Mark as nameref if -n flag was used
            if (declareNameref) {
