...
import { getArrayElement, getArrayIndices, getAssocArrayKeys, } from "../helpers/array.js";
import { quoteArrayValue, quotedArrayValueByteLength, quotedValueByteLength, quoteValue, } from "../helpers/quoting.js";
 ...
 */
function appendArrayOutput(ctx, output, arrayName) {
 ...
 */
function appendAssocKey(output, key) {
 ...
 */
function appendAssocArrayOutput(ctx, output, arrayName) {
 ...
 */
function formatOptionsList(ctx) {
 ...
 */
function formatOptionsResetCommands(ctx) {
...
export function handleSet(ctx, args) {
    ...
    const finish = () => pendingListings.length > 0 ? success(pendingListings.join("")) : OK;
    let i = 0;
    while (i < args.length) {
        const arg = args[i];
        ...
        if (arg.length > 1 &&
            ...
            for (let j = 1; j < arg.length; j++) {
                ...
                if (flag === "o") {
                    ...
                    const hasOptionName = valueIndex < args.length &&
                    ...
                    pendingListings.push(enable ? formatOptionsList(ctx) : formatOptionsResetCommands(ctx));
                    continue;
                }
