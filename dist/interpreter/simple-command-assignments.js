...
import { clearArray, cloneArray, getArray, getArrayElement, parseKeyedElementFromWord, setArrayElement, wordToLiteralString, } from "./helpers/array.js";
...
function appendAssignmentValue(ctx, existing, value) {
    return boundedJoin([existing, value], "", ctx.limits.maxStringLength, "array assignment");
}
/**
...
async function processArrayAssignment(ctx, node, name, array, append, tempAssignments) {
    ...
    const restoreTarget = () => {
        ...
        if (savedArraySnapshot)
            ctx.state.arrays.set(name, savedArraySnapshot);
        else
            ctx.state.arrays.delete(name);
        if (savedScalar === undefined)
            ctx.state.env.delete(name);
        else
            ctx.state.env.set(name, savedScalar);
    };
...
async function processAssociativeArrayAssignment(ctx, node, name, array, append, clearExistingElements, addXtraceOutput) {
    ...
    for (const element of array) {
        if (pendingElements.length >= ctx.limits.maxArrayElements) {
            throw new ExecutionLimitError(`array element limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
        }
        ...
        if (parsed) {
            ...
            if (valueParts.length > 0) {
                ...
                if (valueParts.every((part) => part.type === "Literal" || part.type === "TildeExpansion")) {
    ...
    ctx.executionScope.consumeWork(pendingElements.length, "associative array assignment");
    if (!append) {
    ...
    for (const pending of pendingElements) {
        if (pending.type === "keyed") {
            if (pending.append) {
                ...
                setArrayElement(ctx, name, pending.key, appendAssignmentValue(ctx, existing, pending.value), "associative");
            }
            else {
    ...
    for (const pending of pendingElements) {
        if (pending.type === "keyed") {
            ...
            if (pending.append) {
                ...
                setArrayElement(ctx, name, index, appendAssignmentValue(ctx, existing, pending.value));
            }
            else {
...
async function processSimpleArrayAssignment(ctx, name, array, append, clearExistingElements) {
    ...
    let startIndex = 0;
    ctx.executionScope.consumeWork(allElements.length, "indexed array assignment");
    if (append) {
 ...
 */
async function processSubscriptAssignment(ctx, node, arrayName, subscriptExpr, value, append, _tempAssignments) {
    ...
    const finalValue = append
        ? appendAssignmentValue(ctx, getArrayElement(ctx, resolvedArrayName, elementKey) || "", value)
        : value;
    if (node.name) {
        ...
        setArrayElement(ctx, resolvedArrayName, elementKey, finalValue, isAssoc ? "associative" : "indexed");
    }
    return { continueToNext: true, xtraceOutput: "" };
}
/**
 ...
 */
async function computeAssocArrayKey(ctx, subscriptExpr) {
    let key;
 ...
 */
export async function computeIndexedArrayIndex(ctx, arrayName, subscriptExpr) {
...
async function processScalarAssignment(ctx, node, name, value, append, tempAssignments) {
    ...
    if (node.name) {
        ...
        if (arrayElementKey === undefined)
            ...
            setArrayElement(ctx, targetName, arrayElementKey, finalValue);
 ...
 */
async function computeNamerefArrayKey(ctx, namerefArrayRef) {
