 ...
 */
import { encodeUtf8ToBytes, latin1FromBytes, stdoutAsBytes, } from "../encoding.js";
...
export async function executePipeline(ctx, node, executeCommand) {
    ...
    for (let i = 0; i < node.commands.length; i++) {
        ...
        // charged twice.
        result = ctx.executionScope.accountResult(result, "pipeline", ctx.executionScope.outputBytesUsed - outputCheckpoint);
        if (!isLast) {
            ...
            const releasedStderr = pipeStderrToNext
                ...
                : 0;
            relinquishPipelineOutput(ctx.executionScope, releasedStdout + releasedStderr, "pipeline");
            result = {
                ...result,
    ...
    const effectivePipestatus = lastResult.internalPipeStatusOverride ?? pipestatusExitCodes;
    ...
    const { internalPipeStatusOverride: _internalOverride, ...publicResult } = lastResult;
    return publicResult;
}
