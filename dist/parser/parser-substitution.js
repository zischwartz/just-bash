 ...
 * the delimiter `EOF`.
 */
export function readHeredocDelimiter(value, pos) {
    let delim = "";
    let i = pos;
 ...
 */
function skipHeredocBodies(value, nlIndex, heredocs) {
 ...
 */
function findSubstitutionBodyEnd(value, cmdStart, error) {
    let depth = 1;
    let i = cmdStart;
    ...
    while (i < value.length && depth > 0) {
        ...
        if (inSingleQuote) {
            ...
            if (arithDepth === 0 &&
                ...
                const { delim, endPos, unclosedQuote, unclosedSubstitution } = readHeredocDelimiter(value, p);
                if (unclosedQuote) {
                    error(`unexpected EOF while looking for matching \`${unclosedQuote}'`);
                }
...
export function parseProcessSubstitutionFromString(value, start, createParser, error) {
