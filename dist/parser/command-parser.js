...
import { REDIRECTION_AFTER_FD_VARIABLE, REDIRECTION_AFTER_NUMBER, REDIRECTION_TOKENS, WordParseContext, } from "./types.js";
...
export function parseRedirection(p) {
    ...
    if (opToken.type === TokenType.DLESS ||
        opToken.type === TokenType.DLESSDASH) {
        return parseHeredocStart(p, fd, fdVariable, opToken.type === TokenType.DLESSDASH);
    }
    // Parse target
    if (!p.isWord()) {
...
function parseHeredocStart(p, fd, fdVariable, stripTabs) {
    // Parse delimiter
    if (!p.isWord()) {
    ...
    const redirect = AST.redirection(stripTabs ? "<<-" : "<<", // Use proper here-doc operator
    AST.hereDoc(delimiter, AST.word([]), stripTabs, quoted), fd, fdVariable);
    ...
    while ((!p.isStatementEnd() || p.check(TokenType.RBRACE)) &&
        ...
        if (isRedirection(p)) {
            ...
            if ((endsWithEq || endsWithEqParen) &&
                ...
                const word = p.parseWordFromString(tokenValue, token.quoted, token.singleQuoted, true);
                args.push(p.parseAdjacentWordParts(word.parts, token.end, WordParseContext.Assignment));
            }
        }
        else if (p.isWord()) {
...
function parseAssignment(p) {
    ...
    if (token.end === p.current().start && p.isProcessSubstitutionStart()) {
        wordValue = p.parseAdjacentWordParts(wordValue?.parts ?? [], token.end, WordParseContext.Assignment);
    }
