...
import { ARITH_ASSIGN_OPS, parseAnsiCQuoting, parseArithNumber, parseLocalizationQuoting, parseNestedArithmetic, skipArithWhitespace, } from "./arithmetic-primaries.js";
...
function parseNestedArithExpr(p, input, pos) {
...
function parseArithTernary(p, input, pos) {
    ...
    if (input[currentPos] === "?") {
        currentPos++;
        const { expr: consequent, pos: p2 } = parseNestedArithExpr(p, input, currentPos);
        currentPos = skipArithWhitespace(input, p2);
        if (input[currentPos] === ":") {
            currentPos++;
            const { expr: alternate, pos: p3 } = parseNestedArithExpr(p, input, currentPos);
            return {
...
function parseArithPower(p, input, pos) {
    ...
    if (input.slice(p2, p2 + 2) === "**") {
        ...
        const { expr: exponent, pos: p3 } = p.withDepth(() => parseArithPower(p, input, p2)); // Right associative
        return {
            expr: {
                type: "ArithBinary",
                operator: "**",
...
function parseArithUnary(p, input, pos) {
    ...
    if (input.slice(currentPos, currentPos + 2) === "++" ||
        ...
        const op = input.slice(currentPos, currentPos + 2);
        currentPos += 2;
        const { expr: operand, pos: p2 } = p.withDepth(() => parseArithUnary(p, input, currentPos));
        return {
...
function parseArithPostfix(p, input, pos) {
    ...
    if (input[currentPos] === "[" && expr.type === "ArithConcat") {
        currentPos++; // Skip [
        const { expr: indexExpr, pos: p2 } = parseNestedArithExpr(p, input, currentPos);
        subscript = indexExpr;
        currentPos = p2;
        if (input[currentPos] === "]")
            currentPos++; // Skip ]
    ...
    if (expr.type === "ArithConcat" ||
        ...
        for (const op of ARITH_ASSIGN_OPS) {
            if (input.slice(currentPos, currentPos + op.length) === op &&
                input.slice(currentPos, currentPos + op.length + 1) !== "==") {
                currentPos += op.length;
                const { expr: value, pos: p2 } = p.withDepth(() => parseArithTernary(p, input, currentPos));
...
function parseArithPrimary(p, input, pos, skipAssignment = false) {
    ...
    const nestedResult = parseNestedArithmetic(parseNestedArithExpr, p, input, currentPos);
    if (nestedResult)
        return nestedResult;
