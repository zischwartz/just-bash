...
function normalizeRegexBracketEscapes(pattern) {
    let normalized = "";
...
function parseParameterOperation(p, value, start, _paramName, quoted = false) {
    ...
    if (char === ":") {
        ...
        if ("-=?+".includes(op)) {
            ...
            const wordParts = p.withDepth(() => parseWordParts(p, wordStr, false, false, true, // isAssignment=true for tilde expansion after : in default values
    ...
    if (char === "#" || char === "%") {
        ...
        const patternParts = p.withDepth(() => parseWordParts(p, patternStr, false, false, false));
    ...
    if (char === "/") {
        ...
        if (value[patternEnd] === "/") {
            ...
            const replaceParts = p.withDepth(() => parseWordParts(p, replaceStr, false, false, false));
