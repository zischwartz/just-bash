...
export function createSecureFetch(config) {
    ...
    if (config._dnsResolve) {
        throw new Error("NetworkConfig._dnsResolve is no longer supported: DNS resolution is " +
            "performed by guarded-fetch and cannot be overridden");
    }
    if (config._createConnectionOwner) {
        throw new Error("NetworkConfig._createConnectionOwner is no longer supported: " +
     ...
     */
    function safeHostnameOf(requestUrl) {
        try {
     ...
     * layer runs.
     */
    function buildHostPolicy(requestUrl) {
        if (config.dangerouslyAllowFullInternetAccess) {
    ...
    function isPrivateLiteral(requestUrl) {
        try {
     ...
     */
    async function unguardedFetch(requestUrl, fetchOptions) {
        const init = {
     ...
     */
    function checkPrivateLiteral(url) {
        if (!denyPrivateRanges)
            return;
        let parsed;
        try {
     ...
     */
    function mapGuardedFetchError(error, url, mod, context) {
        if (!mod.isGuardedFetchError(error))
            return error;
        const gfError = error;
        ...
        switch (gfError.code) {
            ...
            case "hostname_unsafe":
                ...
                return onRedirectHop
                    ? new RedirectNotAllowedError(url)
                    : new NetworkAccessDeniedError(url, subReason === "dns_resolution_failed"
    ...
    async function secureFetch(url, options = {}) {
        ...
        const timeoutId = _setTimeout(() => timeoutController.abort(new DOMException("The operation was aborted", "AbortError")), effectiveTimeout);
        const combinedAbort = combineAbortSignals(options.signal, timeoutController.signal);
        try {
            ...
            while (true) {
                ...
                const response = await DefenseInDepthBox.runTrustedAsync(async () => {
                    ...
                    if (credentialsStripped && userHeaders) {
                        const h = userHeaders instanceof Headers
                    ...
                    const mergedHeaders = buildMergedHeaders(userHeaders, firewallHeaders);
                    const fetchOptions = {
                    ...
                    if (!gfModule || isPrivateLiteral(currentUrl)) {
                        if (denyPrivateRanges) {
                            throw new NetworkAccessDeniedError(currentUrl, "DNS pinning unavailable for private IP enforcement");
                        }
                ...
                if (REDIRECT_CODES.has(response.status) && followRedirects) {
                    ...
                    if (!location) {
                        return await responseToResult(response, currentUrl, maxResponseSize, combinedAbort.signal, redirectChain);
                    }
                    ...
                    await awaitWithSignal(cancelResponseBody(response), combinedAbort.signal);
                    ...
                    const rewriteToGet = ((status === 301 || status === 302) && currentMethod === "POST") ||
                        (status === 303 &&
                            currentMethod !== "GET" &&
                            currentMethod !== "HEAD");
                    if (rewriteToGet) {
 ...
 */
async function responseToResult(response, url, maxResponseSize, signal, redirectChain) {
