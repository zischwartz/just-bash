...
function tzShownAsUtc(d, tz) {
    ...
    const shown = new Date(`${get("year")}-${get("month")}-${get("day")}T${String(h).padStart(2, "0")}:${get("minute")}:${get("second")}Z`);
 ...
 */
function parseBareISOInTimezone(s, tz) {
...
export const dateCommand = {
    ...
    async execute(args, ctx) {
        ...
        if (parseTz && !isValidTimezone(parseTz))
            parseTz = undefined;
        ...
        const strftimeLimits = {
            ...
            maxOutputBytes: Math.min(ctx.limits.maxStringLength, ctx.limits.maxOutputSize) - 1,
        };
        if (fmt)
            out = formatStrftime(fmt, ts, displayTz, strftimeLimits);
        else if (iso)
            out = formatStrftime("%Y-%m-%dT%H:%M:%S%z", ts, displayTz, strftimeLimits);
        else if (rfc)
            out = formatStrftime("%a, %d %b %Y %H:%M:%S %z", ts, displayTz, strftimeLimits);
        else
            out = formatStrftime("%a %b %e %H:%M:%S %Z %Y", ts, displayTz, strftimeLimits);
        return { stdout: `${out}\n`, stderr: "", exitCode: 0 };
    },
};
