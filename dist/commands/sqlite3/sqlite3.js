...
const WORKER_TERMINATION_UNACKNOWLEDGED = "Worker termination was not acknowledged";
function assertDatabaseSize(size, maximum) {
...
async function acquireDatabaseLock(fsIdentity, path, signal) {
    ...
    if (lock.poisonedError)
        throw new Error(lock.poisonedError);
    if (lock.held) {
...
async function acquireDatabaseLocks(fsIdentity, paths, signal) {
    const releases = [];
    try {
...
function poisonDatabaseLock(fsIdentity, path, error) {
    const lock = databaseLocks.get(fsIdentity)?.get(path);
    if (!lock || lock.poisonedError)
        return;
...
async function executeInWorker(input, controller, requireDefenseContext) {
    ...
    try {
        ...
        return await new Promise((resolve) => {
            ...
            const settle = async (result, terminate) => {
                if (settled)
                    return;
                settled = true;
                cleanupListeners();
            ...
            const onCancel = bindDefenseContextCallback(requireDefenseContext, "sqlite3", "worker cancellation callback", (reason) => {
                void settle({
                    success: false,
                    error: reason === "abort"
            ...
            if (controller.isCanceled)
                return;
            try {
                worker = DefenseInDepthBox.runTrusted(() => _internals.createWorker(workerPath, input));
            }
            catch (error) {
                controller.close();
                throw error;
            ...
            const onMessage = bindDefenseContextCallback(requireDefenseContext, "sqlite3", "worker message callback", (result) => {
                try {
                    ...
                    void settle(normalizeWorkerResult(result, input.protocolToken), true);
                }
                catch (error) {
                    void settle({
                        success: false,
            ...
            const onExit = bindDefenseContextCallback(requireDefenseContext, "sqlite3", "worker exit callback", (code) => {
                if (code !== 0) {
                    void settle({ success: false, error: `Worker exited with code ${code}` }, false);
                }
            });
            const dispatchMessage = (result) => {
                try {
                    ...
                    void settle({ success: false, error: sanitizeHostErrorMessage(message) }, true);
                }
            };
    ...
    async execute(args, ctx) {
        ...
        try {
            if (!isMemory) {
                ...
                releaseLock = await acquireDatabaseLocks(ctx.fsIdentity ?? ctx.fs, lockPaths, ctx.signal);
                if (await ctx.fs.exists(dbPath)) {
                    ...
                    databaseLease = ctx.executionScope?.reserveBytes("sqlite-database", dbBuffer.byteLength, "sqlite3 database");
                }
            }
        }
        catch (e) {
        ...
        try {
            ...
            requestController.assertMessageSize({ ...workerInput, dbBuffer: undefined }, "request");
            let result;
            let workerCloneLease;
            try {
                if (dbBuffer) {
                    ...
                    assertDatabaseSize(dbBuffer.byteLength, ctx.limits.maxDatabaseBytes);
                    workerCloneLease = ctx.executionScope?.reserveBytes("sqlite-worker-clone", dbBuffer.byteLength, "sqlite3 worker clone");
                }
                result = await executeInWorker(workerInput, requestController, ctx.requireDefenseContext);
            }
            catch (e) {
                const message = sanitizeHostErrorMessage(e.message);
                return {
            ...
            // Format output
            const maxFormattedOutput = Math.min(ctx.limits.maxStringLength, ctx.limits.maxOutputSize);
            const formatOptions = {
                mode: options.mode,
            ...
            for (const stmtResult of result.results) {
                if (stmtResult.type === "error") {
                    ...
                    if (stmtResult.rows.length > 0 || options.header) {
                        try {
                            const remainingOutput = Math.max(0, maxFormattedOutput - Buffer.byteLength(stdout, "utf8"));
            ...
            if (result.hasModifications &&
                ...
                result.dbBuffer) {
                try {
                    assertDatabaseSize(result.dbBuffer.byteLength, ctx.limits.maxDatabaseBytes);
