 ...
 */
export function formatStrftime(format, timestamp, tz, limits = {}) {
    ...
    const append = (value) => {
        ...
        if (limits.maxOutputBytes !== undefined &&
            ...
            throw new ExecutionLimitError(`strftime: output size limit exceeded (${limits.maxOutputBytes} bytes)`, "output_size");
        }
        result += value;
    ...
    while (i < format.length) {
        operations++;
        if (limits.maxOperations !== undefined &&
            operations > limits.maxOperations) {
            throw new ExecutionLimitError(`strftime: iteration limit exceeded (${limits.maxOperations})`, "iterations");
        }
 ...
 */
function formatStrftimeDirective(date, parts, directive, tz) {
 ...
 */
function getWeekNumberForParts(year, month, day, _weekday, startDay) {
