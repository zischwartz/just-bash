...
import { applyWidth, assertFormattingInteger, processEscapes, utf8ByteLength, } from "./escapes.js";
...
export const printfCommand = {
    ...
    async execute(args, ctx) {
        ...
        try {
            const maxOutputBytes = Math.min(ctx.limits.maxStringLength, ctx.limits.maxOutputSize);
            ...
            do {
                const { result, argsConsumed, error, errMsg, stopped } = formatOnce(processedFormat, formatArgs, argPos, tz, maxOutputBytes - outputBytes);
                const resultBytes = utf8ByteLength(result);
                if (resultBytes > maxOutputBytes - outputBytes) {
                    throw new ExecutionLimitError(`printf: output size limit exceeded (${maxOutputBytes} bytes)`, "string_length");
                }
                output += result;
            ...
            if (targetVar) {
                ...
                if (arrayMatch) {
                    ...
                    if (ctx.assignShellVariable) {
                        ...
                        throw new Error("printf -v array assignment requires an interpreter assignment gateway");
                    }
                }
                else {
                    if (ctx.assignShellVariable) {
 ...
 */
function formatOnce(format, args, argPos, tz, maxOutputBytes = 10 * 1024 * 1024) {
    let result = "";
    let i = 0;
    let argsConsumed = 0;
    let error = false;
    ...
    while (i < format.length) {
        if (format[i] === "%" && i + 1 < format.length) {
            ...
            if (strftimeMatch) {
                ...
                formatted = applyWidth(formatted, width, precision, maxOutputBytes - resultBytes);
                append(formatted);
            ...
            const { value, parseError, parseErrMsg, stopped } = formatValue(adjustedSpec, specifier, arg, maxOutputBytes - resultBytes);
            append(value);
            if (parseError) {
                error = true;
                if (parseErrMsg)
 ...
 */
function formatValue(spec, specifier, arg, maxOutputBytes) {
...
function assertFormatSpecBounds(spec, maxOutputBytes) {
 ...
 */
function shellQuote(str, maxOutputBytes) {
    if (str === "") {
        if (maxOutputBytes < 2) {
            throw new ExecutionLimitError(`printf: output size limit exceeded (${maxOutputBytes} bytes)`, "string_length");
        }
 ...
 */
function formatString(spec, str, maxOutputBytes) {
 ...
 */
function formatQuoted(spec, str, maxOutputBytes) {
 ...
 */
function processBEscapes(str, maxOutputBytes) {
    if (utf8ByteLength(str) > maxOutputBytes) {
        throw new ExecutionLimitError(`printf: output size limit exceeded (${maxOutputBytes} bytes)`, "string_length");
    }
    let result = "";
