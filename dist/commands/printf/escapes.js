...
const DEFAULT_MAX_FORMAT_BYTES = 10 * 1024 * 1024;
export function assertFormattingInteger(value, maxBytes, kind) {
    const magnitude = kind === "width" ? Math.abs(value) : value;
    if (!Number.isFinite(value) ||
        !Number.isSafeInteger(value) ||
        magnitude < 0 ||
        magnitude > maxBytes) {
        throw new ExecutionLimitError(`format ${kind} limit exceeded (${maxBytes} bytes)`, "string_length");
    }
}
/**
 ...
 */
export function applyWidth(value, width, precision, maxBytes = DEFAULT_MAX_FORMAT_BYTES) {
    ...
    if (absWidth > result.length) {
        ...
        if (prospectiveBytes > maxBytes) {
            throw new ExecutionLimitError(`formatted value size limit exceeded (${maxBytes} bytes)`, "string_length");
        }
        if (width < 0) {
            // Left-justify
 ...
 */
export function parseWidthPrecision(format, startIndex, maxBytes = DEFAULT_MAX_FORMAT_BYTES) {
    let i = startIndex;
    let width = 0;
    let precision = -1;
 ...
 */
export function processEscapes(str, maxBytes = DEFAULT_MAX_FORMAT_BYTES) {
    if (utf8ByteLength(str) > maxBytes) {
        throw new ExecutionLimitError(`escaped string size limit exceeded (${maxBytes} bytes)`, "string_length");
    }
    let result = "";
    let i = 0;
    while (i < str.length) {
