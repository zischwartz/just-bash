...
export const join = {
    ...
    execute: async (args, ctx) => {
        ...
        const aggregateInputBytes = contents.reduce((total, content) => total + utf8ByteLength(content), 0);
        if (aggregateInputBytes > maxStringLength) {
            throw new ExecutionLimitError(`join: aggregate input size limit exceeded (${maxStringLength} bytes)`, "string_length");
        }
        let parsedLineCount = 0;
        let iterations = 0;
        const useIteration = () => {
            if (++iterations > maxIterations) {
                throw new ExecutionLimitError(`join: iteration limit exceeded (${maxIterations})`, "iterations");
            }
        };
        ...
        const parseLines = (content, joinField) => {
            ...
            for (let i = 0; i < content.length; i++) {
                if (content.charCodeAt(i) === 10 && i + 1 < content.length)
                    lineCount++;
            }
            if (lineCount > maxArrayElements - parsedLineCount) {
                throw new ExecutionLimitError(`join: array element limit exceeded (${maxArrayElements})`, "array_elements");
            }
        ...
        const appendOutput = (line) => {
            ...
            if (lineBytes > maxOutputSize - outputBytes) {
                throw new ExecutionLimitError(`join: output size limit exceeded (${maxOutputSize} bytes)`, "output_size");
            }
            output.push(line);
