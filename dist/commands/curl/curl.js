...
import { applyWriteOut, extractFilename, formatHeaderBlock, } from "./response-formatting.js";
/**
...
function buildOutput(options, result, requestUrl) {
    ...
    if (options.includeHeaders && !options.verbose) {
        output += formatHeaderBlock(result.status, result.statusText, result.headers);
    }
...
export const curlCommand = {
    ...
    async execute(args, ctx) {
        ...
        try {
            ...
            const finalHeaderBlock = formatHeaderBlock(result.status, result.statusText, result.headers);
            ...
            if (result.redirectChain) {
                for (const hop of result.redirectChain) {
                    headerBlock += formatHeaderBlock(hop.status, hop.statusText, hop.headers);
                }
            }
