...
import { decodeBytesToUtf8, latin1FromBytes, readBytesFrom, unsafeBytesFromLatin1, utf8ByteLength, } from "../../encoding.js";
...
function reservePatternSource(ctx, bytes) {
    ...
    if (!Number.isSafeInteger(bytes) ||
        ...
        throw new ExecutionLimitError(bytes > ctx.limits.maxStringLength
            ? `rg: pattern file exceeds string limit (${ctx.limits.maxStringLength} bytes)`
            : `rg: live byte limit exceeded (${ctx.limits.maxLiveBytes} bytes)`, "string_length");
    }
    return (ctx.executionScope?.reserveBytes("rg pattern files", bytes * 3, "rg") ?? {
        release: () => undefined,
    });
}
function appendPatternLines(patterns, content, ctx) {
    let lineStart = 0;
    for (let index = 0; index <= content.length; index++) {
        ...
        if (index > lineStart) {
            if (patterns.length >= ctx.limits.maxArrayElements) {
                throw new ExecutionLimitError(`rg: pattern limit exceeded (${ctx.limits.maxArrayElements})`, "iterations");
            }
...
function accountPatternInput(ctx, bytes, aggregateBytes) {
    if (bytes > ctx.limits.maxInputBytes - aggregateBytes) {
        throw new ExecutionLimitError(`rg: aggregate input size limit exceeded (${ctx.limits.maxInputBytes} bytes)`, "string_length");
    }
...
export async function executeSearch(searchCtx) {
    ...
    try {
        ...
        for (const patternFile of options.patternFiles) {
            try {
                ...
                if (patternFile === "-") {
                    ...
                    aggregatePatternInputBytes = accountPatternInput(ctx, contentBytes, aggregatePatternInputBytes);
                }
                else {
                    ...
                    if (actualBytes > contentBytes) {
                        throw new ExecutionLimitError("rg: pattern file grew while being read", "string_length");
                    }
                ...
                ctx.executionScope?.consumeWork(content.length, "rg pattern file parsing");
        ...
        for (const lease of patternSourceLeases)
            lease.release();
    }
            ...
            if (stat.isFile) {
                ...
                if (shouldIncludeFile(path, options, gitignore, fullPath, typeRegistry)) {
                    if (files.length >= ctx.limits.maxArrayElements) {
                        throw new ExecutionLimitError(`rg: file collection limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
                    }
                    files.push(path);
                }
            }
                ...
                directoryCount++;
                await walkDirectory(ctx, path, fullPath, 0, options, gitignore, typeRegistry, files, traversalBudget, new Set());
            }
        }
        catch (error) {
            rethrowFatalExecutionError(error);
 ...
 */
async function walkDirectory(ctx, relativePath, absolutePath, depth, options, gitignore, typeRegistry, files, budget, activeDirectories) {
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            if (isDirectory) {
                await walkDirectory(ctx, entryRelativePath, entryAbsolutePath, depth + 1, options, gitignore, typeRegistry, files, budget, activeDirectories);
            }
            else if (isFile) {
...
async function listFiles(ctx, inputPaths, options) {
    ...
    const output = new BoundedStringBuilder(Math.min(ctx.limits.maxOutputSize, ctx.limits.maxStringLength), "rg");
    for (const file of files)
        output.append(file).append(sep);
...
async function readFileContent(ctx, filePath, file, options) {
    ...
    try {
        ...
        if (options.preprocessor && ctx.exec) {
            ...
            if (matchesPreGlob(filename, options.preprocessorGlobs)) {
                ...
                if (result.exitCode === 0 && result.stdout) {
                    ...
                    lease = ctx.executionScope?.reserveBytes("rg preprocessor text", result.stdout.length, "rg preprocessor");
                    const content = decodeBytesToUtf8(unsafeBytesFromLatin1(result.stdout));
                    ctx.executionScope?.consumeInput(utf8ByteLength(content), "rg preprocessor");
        ...
        if (options.searchZip && file.endsWith(".gz")) {
            const stat = await ctx.fs.stat(filePath);
            const inputLease = ctx.executionScope?.reserveBytes("rg compressed input", stat.size, "rg");
            const buffer = await ctx.fs.readFileBuffer(filePath);
            if (buffer.byteLength > stat.size) {
                inputLease?.release();
                throw new ExecutionLimitError("rg: file grew while being read", "string_length");
            }
            ...
            if (isGzip(buffer)) {
                ...
                try {
                    ...
                    const outputCapacity = Math.min(ctx.limits.maxStringLength, ctx.limits.maxOutputSize, Math.floor((ctx.executionScope?.remainingLiveBytes ??
                        ctx.limits.maxLiveBytes) / 2));
                    outputLease = ctx.executionScope?.reserveBytes("rg decompressed text", outputCapacity * 2, "rg");
        ...
        lease = ctx.executionScope?.reserveBytes("rg file text", stat.size * 2, "rg");
...
function compositeLease(...leases) {
    const active = leases.filter((item) => item !== undefined);
    if (active.length === 0)
        return undefined;
    return {
        release: () => {
