...
function parseRange(spec, maximum) {
    const ranges = [];
    const parts = spec.split(",");
    if (parts.length > maximum) {
        throw new ExecutionLimitError(`cut: range count limit exceeded (${maximum})`, "array_elements");
    }
...
export const cutCommand = {
    ...
    async execute(args, ctx) {
        ...
        const ranges = parseRange(fieldSpec || charSpec || "1", ctx.limits.maxArrayElements);
        const output = new BoundedStringBuilder(Math.min(ctx.limits.maxOutputSize, ctx.limits.maxStringLength), "cut");
        let rangeWork = 0;
        const chargeRangeWork = (count) => {
            if (!Number.isSafeInteger(count) ||
                count < 0 ||
                count > ctx.limits.maxLoopIterations - rangeWork) {
                throw new ExecutionLimitError(`cut: range expansion limit exceeded (${ctx.limits.maxLoopIterations})`, "iterations");
            }
            rangeWork += count;
