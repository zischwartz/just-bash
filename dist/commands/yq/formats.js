 ...
 */
export function parseInput(input, options, limits = {}) {
    const trimmed = input.trim();
    if (!trimmed)
        return null;
    const sanitize = (value) => sanitizeParsedData(value, limits);
    switch (options.inputFormat) {
        ...
        case "csv":
            // SECURITY: sanitizeParsedData converts to null-prototype objects at boundary.
            return sanitize(parseCsv(trimmed, options.csvDelimiter, options.csvHeader));
        case "toml":
            // SECURITY: sanitizeParsedData converts to null-prototype objects at boundary.
 ...
 */
export function parseAllYamlDocuments(input, limits = {}) {
    ...
    while (lineStart < input.length) {
        ...
        if (lineStart > 0 &&
            ...
            if (documents > maxDocuments) {
                throw new ExecutionLimitError(`query input document limit exceeded (${maxDocuments})`, "array_elements");
            }
        }
        if (lineEnd === -1)
            break;
    ...
    for (const doc of docs) {
        values.push(sanitizeParsedData(doc.toJS({ maxAliasCount: 100 }), {
            ...limits,
            elementBudget,
        }));
    }
 ...
 */
export function extractFrontMatter(input, limits = {}) {
    ...
    if (trimmed.startsWith("---")) {
        ...
        if (endMatch && endMatch.index !== undefined) {
            ...
            return {
                frontMatter: sanitizeParsedData(YAML.parse(yamlContent, { maxAliasCount: 100 }), limits),
                content: remaining,
            };
        }
    }
    ...
    if (trimmed.startsWith("+++")) {
        ...
        if (endMatch && endMatch.index !== undefined) {
            ...
            const remaining = trimmed.slice(endMatch.index + 3 + endMatch[0].length);
            return {
                frontMatter: sanitizeParsedData(TOML.parse(tomlContent), limits),
                content: remaining,
            };
        }
    }
 ...
 */
export function formatOutput(value, options, maxBytes = Number.MAX_SAFE_INTEGER) {
    if (value === undefined)
        return "";
    ...
    switch (options.outputFormat) {
        ...
        case "ini": {
            ...
            serialized = ini.stringify(value);
            break;
        }
        case "csv":
        ...
        case "toml": {
            ...
            serialized = TOML.stringify(value);
            break;
        }
        default:
            throw new Error(`Unknown output format: ${options.outputFormat}`);
    }
    const bounded = new BoundedStringBuilder(maxBytes, "yq output", () => new ExecutionLimitError(`output size limit exceeded (${maxBytes} bytes)`, "output_size"));
 ...
 */
function assertExternalSerializationFits(value, options, maxBytes) {
    const escapeFactor = options.outputFormat === "yaml" || options.outputFormat === "toml" ? 6 : 2;
    const active = new WeakSet();
    const pending = [{ kind: "value", value, depth: 0, labelBytes: 0 }];
    let estimated = 0;
    const add = (bytes) => {
        if (!Number.isSafeInteger(bytes) ||
            bytes < 0 ||
            bytes > maxBytes - estimated) {
            throw new ExecutionLimitError(`output size limit exceeded (${maxBytes} bytes)`, "output_size");
        }
        estimated += bytes;
    };
    while (pending.length > 0) {
        ...
        if (current === null || typeof current !== "object") {
            add(labelBytes + depth * options.indent + utf8ByteLength(String(current)));
            continue;
        }
