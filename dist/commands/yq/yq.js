...
function parseIndent(value) {
    ...
    if (!Number.isSafeInteger(indent) || indent < 0 || indent > 32)
        return null;
    return indent;
}
function invalidIndent(value) {
    return {
        stdout: "",
...
function parseArgs(args) {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (a.startsWith("--input-format=")) {
            ...
            if (indent === null)
                return invalidIndent(indentValue);
...
export const yqCommand = {
    ...
    async execute(args, ctx) {
        ...
        try {
            ...
            // Format output
            const maxOutputSize = Math.min(ctx.limits.maxStringLength, ctx.limits.maxOutputSize);
            const separator = options.joinOutput ? "" : "\n";
            const output = new BoundedStringBuilder(maxOutputSize, "yq output", () => new ExecutionLimitError(`output size limit exceeded (${maxOutputSize} bytes)`, "output_size"));
            ...
            for (const value of values) {
                ...
                const remainingBytes = output.remainingBytes - separatorBytes - finalNewlineBytes;
                if (remainingBytes < 0) {
                    throw new ExecutionLimitError(`output size limit exceeded (${maxOutputSize} bytes)`, "output_size");
                }
                const serializationLimit = Math.min(remainingBytes, ctx.executionScope?.remainingLiveBytes ?? remainingBytes);
                const serializationLease = ctx.executionScope?.reserveBytes("yq serialization", serializationLimit, "yq output");
                let text;
                try {
            ...
            if (formattedValues > 0 && !options.joinOutput)
                output.append("\n");
