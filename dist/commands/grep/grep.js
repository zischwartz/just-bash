...
function getMatcherWorkLimit(ctx) {
...
function useTraversalOperation(budget) {
    if (++budget.operations > budget.maxOperations) {
        throw new ExecutionLimitError(`grep: glob operation limit exceeded (${budget.maxOperations})`, "glob_operations");
    }
}
function addTraversalResult(budget) {
    if (budget.results >= budget.maxResults) {
        throw new ExecutionLimitError(`grep: array element limit exceeded (${budget.maxResults})`, "array_elements");
    }
    budget.results++;
}
/**
 ...
 */
function splitPatternOperand(value) {
    return value.split("\n");
}
/**
 ...
 */
function splitPatternFile(content) {
    if (content === "")
        return [];
 ...
 */
function combinePatterns(patterns, mode) {
...
export const grepCommand = {
    ...
    async execute(args, ctx) {
        ...
        const patterns = pattern === null ? [] : splitPatternOperand(pattern);
        ...
        for (const patternFile of patternFiles) {
            ...
            if (patterns.length + filePatterns.length > ctx.limits.maxArrayElements) {
                throw new ExecutionLimitError(`grep: array element limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
            }
        ...
        const combined = patterns.length === 0
            ? { pattern: NEVER_MATCHES, mode: "extended" }
        ...
        const appendFiles = (entries) => {
            if (entries.length > traversalBudget.maxResults - filesToSearch.length) {
                throw new ExecutionLimitError(`grep: array element limit exceeded (${traversalBudget.maxResults})`, "array_elements");
            }
        ...
        for (const file of files) {
            ...
            if (file.includes("*") || file.includes("?") || file.includes("[")) {
                const expanded = await expandGlobPatternWithTypes(file, ctx, traversalBudget);
                if (recursive) {
                    for (const f of expanded) {
                        const recursiveExpanded = await expandRecursiveWithTypes(f.path, ctx, includePatterns, excludePatterns, excludeDirPatterns, f.isFile, traversalBudget);
                ...
                const expanded = await expandRecursiveWithTypes(file, ctx, includePatterns, excludePatterns, excludeDirPatterns, undefined, traversalBudget);
                appendFiles(expanded);
            }
            else {
                if (filesToSearch.length >= traversalBudget.maxResults) {
        ...
        const showFilename = (filesToSearch.length > 1 || (recursive && hasFileTarget)) && !noFilename;
...
async function expandRecursiveGlob(baseDir, afterGlob, ctx, result, budget, depth = 0) {
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            if (entryStat.isDirectory) {
                ...
                await expandRecursiveGlob(entryPath, afterGlob, ctx, result, budget, depth + 1);
            }
            else if (afterGlob) {
 ...
 */
async function expandGlobPatternWithTypes(pattern, ctx, budget) {
    const result = [];
 ...
 * Uses readdirWithFileTypes when available to avoid stat calls
 */
async function expandRecursiveWithTypes(path, ctx, includePatterns = [], excludePatterns = [], excludeDirPatterns = [], knownIsFile, budget = {
    operations: 0,
    results: 0,
    maxOperations: ctx.limits.maxGlobOperations,
    maxResults: ctx.limits.maxArrayElements,
    ...
    try {
        ...
        if (ctx.fs.readdirWithFileTypes) {
            ...
            for (const entry of entries) {
                ...
                await expandRecursiveWithTypes(entryPath, ctx, includePatterns, excludePatterns, excludeDirPatterns, entry.isFile, budget, result, depth + 1);
            }
        }
        else {
            useTraversalOperation(budget);
            ...
            for (const entry of entries) {
                ...
                await expandRecursiveWithTypes(entryPath, ctx, includePatterns, excludePatterns, excludeDirPatterns, undefined, budget, result, depth + 1);
            }
        }
    }
    catch (error) {
