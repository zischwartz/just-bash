...
function escapeControlChar(char) {
    switch (char) {
        case "\b":
            return "\\b";
        case "\f":
...
function parseJsonSlice(input, startPos, endPos) {
 ...
 */
function parseJsonStream(input, limits) {
    const results = [];
    const appendResult = (value) => {
        if (results.length >= limits.maxElements) {
            throw new ExecutionLimitError(`query result element limit exceeded (${limits.maxElements})`, "array_elements");
        }
        results.push(value);
    };
    let pos = 0;
    ...
    while (pos < len) {
        ...
        if (char === "{" || char === "[") {
            ...
            if (bracketStack.length !== 0) {
                throw new Error(`Unexpected end of JSON input at position ${pos} (unclosed ${char})`);
            }
            appendResult(sanitizeParsedData(parseJsonSlice(input, startPos, pos), {
            ...
            appendResult(sanitizeParsedData(parseJsonSlice(input, startPos, pos), limits));
        }
...
export const jqCommand = {
    ...
    async execute(args, ctx) {
        ...
        for (let i = 0; i < args.length; i++) {
            ...
            if (a === "-R" || a === "--raw-input")
                ...
                if (name === undefined || value === undefined) {
                    return jqArgError("--arg takes two parameters (e.g. --arg varname value)");
                }
                ...
                if (name === undefined || json === undefined) {
                    return jqArgError("--argjson takes two parameters (e.g. --argjson varname text)");
                }
                let parsed;
                try {
                ...
                if (name === undefined || file === undefined) {
                    return jqArgError("--rawfile takes two parameters (e.g. --rawfile varname filename)");
                }
                ...
                const file = args[i + 2];
                if (name === undefined || file === undefined) {
                    return jqArgError("--slurpfile takes two parameters (e.g. --slurpfile varname filename)");
                }
        ...
        if (fileBindings.length > 0) {
            const result = await withDefenseContext("arg file read", () => readFiles(ctx, fileBindings.map((b) => b.file), { cmdName: "jq", stopOnError: true }));
            ...
            for (let b = 0; b < fileBindings.length; b++) {
                ...
                if (mode === "raw") {
                    ...
                    try {
                        namedArgs.set(name, trimmed ? parseJsonStream(trimmed, jsonLimits) : []);
                    }
                    catch {
        ...
        try {
            ...
            const appendValues = (target, next) => {
                if (next.length > ctx.limits.maxQueryElements - target.length) {
                    throw new ExecutionLimitError(`query result element limit exceeded (${ctx.limits.maxQueryElements})`, "array_elements");
                }
            ...
            if (nullInput) {
                ...
                for (const { content } of inputs) {
                    ...
                    let nl = text.indexOf("\n", start);
                    while (nl !== -1) {
                        appendValues(values, evaluate(text.slice(start, nl), ast, evalOptions));
                        start = nl + 1;
                        nl = text.indexOf("\n", start);
                    }
            ...
            const maxStringLength = Math.min(ctx.limits.maxStringLength, ctx.limits.maxOutputSize);
            const formatted = [];
            let outputBytes = 0;
            for (const value of values) {
                const separatorBytes = formatted.length > 0 ? separator.length : 0;
                const finalNewlineBytes = joinOutput ? 0 : 1;
                const remainingBytes = maxStringLength - outputBytes - separatorBytes - finalNewlineBytes;
                if (remainingBytes < 0) {
                    throw new ExecutionLimitError(`output size limit exceeded (${maxStringLength} bytes)`, "string_length");
                }
                if (getValueDepth(value, ctx.limits.maxQueryDepth + 1) >
                    ctx.limits.maxQueryDepth) {
                    throw new ExecutionLimitError(`query depth limit exceeded (${ctx.limits.maxQueryDepth})`, "recursion");
                }
                ...
                if (outputBytes + separatorBytes + textBytes + finalNewlineBytes >
                    maxStringLength) {
                    throw new ExecutionLimitError(`output size limit exceeded (${maxStringLength} bytes)`, "string_length");
                }
