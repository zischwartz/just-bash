...
export function formatJsonValue(value, maxBytes, options = {}) {
    const output = new BoundedStringBuilder(maxBytes, "query JSON output", () => new ExecutionLimitError(`output size limit exceeded (${maxBytes} bytes)`, options.limitKind ?? "output_size"));
    appendJsonValue(output, value, options, 0);
    return output.build();
}
function appendJsonValue(output, value, options, depth) {
    ...
    if (Array.isArray(value)) {
        ...
        for (let index = 0; index < value.length; index++) {
            output.append(compact ? (index === 0 ? "" : ",") : index === 0 ? "\n" : ",\n");
            if (!compact)
                output.repeat(indentUnit, depth + 1);
...
function appendJsonString(output, value) {
    output.append('"');
    let start = 0;
    for (let index = 0; index < value.length; index++) {
