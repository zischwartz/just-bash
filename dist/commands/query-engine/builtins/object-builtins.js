...
function maxResultElements(ctx) {
    return ctx.limits.maxArrayElements;
}
function assertResultPush(ctx, length) {
    const limit = maxResultElements(ctx);
    if (length >= limit) {
        throw new ExecutionLimitError(`query result element limit exceeded (${limit})`, "array_elements");
    }
}
function validateStreamPath(path, ctx) {
    ...
    switch (name) {
        ...
        case "to_entries": {
            ...
            if (toEntriesObj) {
                ...
                if (keys.length > maxResultElements(ctx)) {
                    throw new ExecutionLimitError(`query result element limit exceeded (${maxResultElements(ctx)})`, "array_elements");
                }
                return [
                    keys.map((key) => {
        ...
        case "tostream": {
            ...
            while (stack.length > 0) {
                ...
                if (++iterations > ctx.limits.maxIterations) {
                    throw new ExecutionLimitError(`query iteration limit exceeded (${ctx.limits.maxIterations})`, "iterations");
                }
                ...
                if (v === null || typeof v !== "object") {
                    ...
                    if (seen.has(v)) {
                        throw new ExecutionLimitError("cyclic value cannot be converted to a stream", "recursion");
                    }
                    seen.add(v);
                    if (v.length === 0) {
                        ...
                        if (stack.length > maxResultElements(ctx) - v.length) {
                            throw new ExecutionLimitError(`query traversal queue limit exceeded (${maxResultElements(ctx)})`, "array_elements");
                        }
        ...
        case "fromstream": {
            ...
            if (streamItems.length > maxResultElements(ctx)) {
                throw new ExecutionLimitError(`query stream item limit exceeded (${maxResultElements(ctx)})`, "array_elements");
            }
            let result = null;
            let iterations = 0;
