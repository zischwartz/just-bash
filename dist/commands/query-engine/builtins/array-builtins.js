...
function assertResultCapacity(ctx, current, additional = 1) {
    const maxElements = ctx.limits.maxArrayElements;
    if (additional < 0 || current > maxElements - additional) {
        throw new QueryExecutionLimitError(`query result element limit exceeded (${maxElements})`, "array_elements");
    }
}
/**
