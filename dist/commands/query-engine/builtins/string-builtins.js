...
function countLiteralSplit(value, separator, limit) {
    if (value === "")
        ...
        case "join": {
            ...
            return seps.map((sep) => {
                ...
                for (let i = 0; i < value.length; i++) {
                    ...
                    if (i > 0)
                        bytes += utf8ByteLength(separator);
                    if (bytes > stringLimit(ctx)) {
                        throw new ExecutionLimitError(`query string size limit exceeded (${stringLimit(ctx)} bytes)`, "string_length");
                    }
                }
                return value
