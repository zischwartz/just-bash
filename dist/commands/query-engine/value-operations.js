...
export function deepMerge(a, b, options = {}) {
    ...
    const stack = [{ target: root, source: b, depth: 0 }];
...
export function getValueDepth(value, maxCheck = 3000) {
    ...
    if (maxCheck <= 1)
        return maxCheck;
    const createFrame = (current, pathDepth) => ({
        value: current,
    ...
    const childCount = (frame) => frame.keys !== undefined
        ? frame.keys.length
        : frame.value.length;
    const nextChild = (frame) => {
        ...
        return frame.value[frame.nextChild++];
    };
    ...
    const root = value;
    const stack = [createFrame(root, 1)];
