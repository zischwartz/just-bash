...
function queryLimitError(message, kind) {
    return new ExecutionLimitError(message, kind);
}
export function chargeQueryWork(ctx, count = 1) {
    if (!Number.isSafeInteger(count) ||
        count < 0 ||
        ctx.budget.operations > ctx.limits.maxIterations - count) {
        throw queryLimitError(`query evaluation: too many iterations (${ctx.limits.maxIterations})`, "iterations");
    }
    ctx.budget.operations += count;
}
export function assertQueryResultCapacity(ctx, current, additional = 1) {
    ...
    if (!Number.isSafeInteger(current) ||
        ...
        throw queryLimitError(`query result element limit exceeded (${maxElements})`, "array_elements");
    }
}
function appendQueryResults(ctx, target, values) {
    assertQueryResultCapacity(ctx, target.length, values.length);
    for (const value of values)
        target.push(value);
}
function boundedFlatMap(ctx, values, mapper) {
    const results = [];
    for (const value of values) {
        chargeQueryWork(ctx);
...
function createContext(options) {
    ...
    return {
        ...
        limits: {
            ...
            maxStringLength: options?.limits?.maxStringLength ?? DEFAULT_MAX_STRING_LENGTH,
            maxOutputSize: options?.limits?.maxOutputSize ?? DEFAULT_MAX_STRING_LENGTH,
...
export function evaluate(value, ast, ctxOrOptions) {
    ...
    if (ctx.budget.callDepth > ctx.limits.maxDepth) {
        ctx.budget.callDepth--;
        throw queryLimitError(`query depth limit exceeded (${ctx.limits.maxDepth})`, "recursion");
    }
    try {
...
function evaluateNode(value, ast, ctx) {
    ...
    switch (ast.type) {
        ...
        case "Slice": {
            ...
            return boundedFlatMap(ctx, bases, (v) => {
                ...
                return boundedFlatMap(ctx, starts, (s) => ends.map((e) => {
        ...
        case "Object": {
            ...
            for (const entry of ast.entries) {
                ...
                const newResults = [];
                const safeKeyCount = keys.filter((key) => typeof key === "string" && isSafeKey(key)).length;
        ...
        case "StringInterp": {
            const output = new BoundedStringBuilder(ctx.limits.maxStringLength, "query string interpolation", () => new ExecutionLimitError(`string size limit exceeded (${ctx.limits.maxStringLength} bytes)`, "string_length"));
                ...
                if (typeof idx === "number") {
                    ...
                    if (Array.isArray(val)) {
                        ...
                        if (i >= 0) {
                            ...
                            if (arr.length <= i)
                                arr.length = i + 1;
                            for (let fill = val.length; fill < i; fill++)
                                arr[fill] = null;
                            arr[i] = transform(arr[i]);
                        }
                        return arr;
                    ...
                    if (val === null || val === undefined) {
                        ...
                        const arr = new Array(idx + 1).fill(null);
            ...
            case "+":
                ...
                if (typeof l === "string" && typeof r === "string") {
                    ...
                    if (combinedBytes > maxStringLength) {
                        throw new ExecutionLimitError(`string size limit exceeded (${maxStringLength} bytes)`, "string_length");
                    }
                    return l + r;
                }
