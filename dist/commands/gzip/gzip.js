...
function gunzipWithLimit(inputData, maxDecompressedSize, maxCompressedSize, signal, ctx) {
    ...
    let outputLease;
    try {
        const effectiveMaxOutput = Math.min(maxDecompressedSize, Math.floor((ctx?.executionScope?.remainingLiveBytes ?? maxDecompressedSize * 2) /
            2));
        ...
        outputLease = ctx?.executionScope?.reserveBytes("gzip output", effectiveMaxOutput * 2, "gzip");
...
function gzipWithLimit(inputData, level, ctx) {
    const budget = new CodecBudget({
    ...
    let outputLease;
    try {
        const effectiveMaxOutput = Math.min(ctx.limits.maxOutputSize, Math.floor((ctx.executionScope?.remainingLiveBytes ??
...
async function readReservedFile(ctx, path, site) {
    const stat = await ctx.fs.stat(path);
    if (stat.size > ctx.limits.maxInputBytes) {
        throw new ExecutionLimitError(`input size limit exceeded (${ctx.limits.maxInputBytes} bytes)`, "string_length");
    }
    ...
    try {
        ...
        if (data.byteLength > stat.size) {
            throw new ExecutionLimitError("gzip: input grew while being read", "string_length");
        }
...
async function readReservedOperand(ctx, file, site) {
    if (file !== "-" && file !== "") {
        return readReservedFile(ctx, ctx.fs.resolvePath(ctx.cwd, file), site);
    }
    const stdinBytes = latin1FromBytes(ctx.stdin);
    const lease = ctx.executionScope?.reserveBytes("gzip input", stdinBytes.length, site);
    try {
        return {
...
async function processFile(ctx, file, flags, cmdName, decompress, toStdout) {
    ...
    if (file === "-" || file === "") {
        const stdinBytes = latin1FromBytes(ctx.stdin);
        const inputLease = ctx.executionScope?.reserveBytes("gzip input", stdinBytes.length, cmdName);
        let codecLease;
        try {
            ...
            if (decompress) {
                ...
                try {
                    const decoded = gunzipWithLimit(inputData, maxDecompressedSize, ctx.limits.maxInputBytes, ctx.signal, ctx);
                ...
                const encoded = gzipWithLimit(inputData, getCompressionLevel(flags), ctx);
...
async function processDirectory(ctx, dirPath, flags, cmdName, decompress, toStdout) {
    const maxOutput = Math.min(ctx.limits.maxOutputSize, ctx.limits.maxStringLength);
    ...
    await traverseFileTree({
        ...
        const suffix = flags.suffix;
        if (decompress && !entry.path.endsWith(suffix))
            return;
        if (!decompress && entry.path.endsWith(suffix))
            return;
...
async function testFile(ctx, file, flags, cmdName) {
    ...
    try {
        ...
        try {
            const decoded = gunzipWithLimit(inputData, getMaxDecompressedSize(ctx), ctx.limits.maxInputBytes, ctx.signal, ctx);
