...
function processContent(content, options, currentNumber, maxOutputBytes) {
    // Handle empty input
    if (content === "") {
    ...
    for (const line of lines) {
        ...
        const suffixBytes = utf8ByteLength(options.separator) + utf8ByteLength(line);
        if (shouldNumber(line, options.bodyStyle)) {
            const prospectiveBytes = Math.max(String(lineNumber).length, options.width) +
                suffixBytes +
                newlineBytes;
            if (prospectiveBytes > maxOutputBytes - outputBytes) {
                throw new ExecutionLimitError(`nl: output size limit exceeded (${maxOutputBytes} bytes)`, "output_size");
            }
...
export const nl = {
    ...
    execute: async (args, ctx) => {
        ...
        const files = [];
        const maxFieldWidth = Math.min(ctx.limits.maxStringLength, ctx.limits.maxOutputSize);
        ...
        if (files.length === 0) {
            ...
            for (const file of files) {
                ...
                const result = processContent(content, options, lineNumber, maxOutputBytes - utf8ByteLength(output));
                output += result.output;
