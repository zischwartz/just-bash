...
function parseSize(sizeStr) {
    ...
    if (num > Math.floor(Number.MAX_SAFE_INTEGER / multiplier))
        return null;
...
function identityUnchanged(initial, current) {
    ...
    if (initial.existence === "existing" && current.existence === "existing") {
        return (initial.stableIdentity !== undefined &&
            initial.stableIdentity === current.stableIdentity);
    }
    return false;
}
async function uniqueSiblingPath(ctx, outputPath, purpose) {
    ...
    for (let attempts = 0; attempts < 100; attempts++) {
        ...
        const candidate = ctx.fs.resolvePath(parent, `.${basename}.just-bash-split-${purpose}-${id}`);
        const identity = await resolveFileIdentity(ctx.fs, candidate);
        if (identity.existence === "missing")
            return candidate;
        if (identity.existence === "unknown")
            break;
    }
...
export const split = {
    ...
    execute: async (args, ctx) => {
        ...
        if (utf8ByteLength(prefix) > ctx.limits.maxStringLength ||
            ...
            options.suffixLength >
                ctx.limits.maxStringLength -
                    ...
                    Math.min(ctx.limits.maxStringLength, utf8ByteLength(options.additionalSuffix))) {
            return {
                exitCode: 1,
                stdout: "",
        ...
        if (inputFile === "-") {
            ...
            contentLease = ctx.executionScope?.reserveBytes("split transaction", inputLength * 2, "split");
            ...
            try {
                ...
                contentLease = ctx.executionScope?.reserveBytes("split transaction", stat.size * 2, "split");
                content = await ctx.fs.readFileBuffer(inputPath);
                if (content.byteLength > stat.size) {
                    throw new ExecutionLimitError("split: input grew while being read", "string_length");
                }
        ...
        try {
            ...
            const capacity = suffixCapacity(options.useNumericSuffix, options.suffixLength);
            ...
            ctx.executionScope?.consumeWork(chunks.length * options.suffixLength, "split suffix generation");
            ...
            try {
                ...
                if (inputPath !== null &&
                    inputIdentity !== null &&
                    !identityUnchanged(inputIdentity, await resolveFileIdentity(ctx.fs, inputPath))) {
                ...
                for (const output of outputs) {
                    ...
                    if (inputIdentity?.existence === "existing" &&
                ...
                for (const output of outputs) {
                    ...
                    if (!identityUnchanged(output.initialIdentity, await resolveFileIdentity(ctx.fs, output.path))) {
                        throw new Error("output identity changed during split");
                    }
                    if (output.initialIdentity.existence === "existing") {
                        output.backupPath = await uniqueSiblingPath(ctx, output.path, "backup");
