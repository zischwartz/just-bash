...
function appendLsOutput(ctx, current, next) {
    if (utf8ByteLength(next) >
        ctx.limits.maxOutputSize - utf8ByteLength(current)) {
        throw new ExecutionLimitError(`ls: output size limit exceeded (${ctx.limits.maxOutputSize} bytes)`, "output_size");
    }
    return current + next;
}
function joinLsLines(ctx, lines) {
...
export const lsCommand = {
    ...
    async execute(args, ctx) {
        ...
        const sortKey = resolveSortKey(args, parsed.result.flags.sortBySize, parsed.result.flags.sortByTime);
        ...
        if (directoryOnly) {
            for (const path of await sortOperands(paths, ctx, sortKey, reverse, traversalBudget)) {
                ...
                try {
                    ...
                    if (longFormat) {
                        ...
                        stdout = appendLsOutput(ctx, stdout, `${mode} 1 user user ${sizeStr} ${dateStr} ${path}${suffix}\n`);
                    }
                    else {
                        const suffix = classifyFiles
                            ? classifySuffix(await ctx.fs.lstat(fullPath))
                    ...
                    stderr = appendLsOutput(ctx, stderr, `ls: cannot access '${path}': No such file or directory\n`);
                    exitCode = 2;
                }
            }
        ...
        for (const path of paths) {
            ...
            try {
                ...
                stderr = appendLsOutput(ctx, stderr, `ls: ${path}: No such file or directory\n`);
                exitCode = 2;
            }
        }
        const listOperand = async (path, showHeader) => {
            const result = await listPath(path, ctx, showAll, showAlmostAll, longFormat, recursive, showHeader, reverse, humanReadable, sortKey, classifyFiles, false, traversalBudget, 0, new Set(), true);
            stdout = appendLsOutput(ctx, stdout, result.stdout);
            stderr = appendLsOutput(ctx, stderr, result.stderr);
            if (result.exitCode !== 0)
                exitCode = result.exitCode;
        };
        for (const path of await sortOperands(fileOperands, ctx, sortKey, reverse, traversalBudget)) {
        ...
        for (const path of await sortOperands(dirOperands, ctx, sortKey, reverse, traversalBudget)) {
            if (stdout)
 ...
 */
function resolveSortKey(args, sortBySize, sortByTime) {
    if (!sortBySize && !sortByTime)
        return "name";
    if (sortBySize !== sortByTime)
        return sortBySize ? "size" : "time";
    let last = "name";
...
async function sortOperands(paths, ctx, sortKey, reverse, traversalBudget) {
    return await sortNames(paths, (path) => ctx.fs.resolvePath(ctx.cwd, path), ctx, sortKey, reverse, traversalBudget);
}
/**
 ...
 */
async function sortNames(names, resolve, ctx, sortKey, reverse, traversalBudget) {
    const sorted = [...names];
    if (sortKey === "name") {
        sorted.sort();
    }
...
async function listPath(path, ctx, showAll, showAlmostAll, longFormat, recursive, showHeader, reverse = false, humanReadable = false, sortKey = "name", classifyFiles = false, _isSubdir = false, traversalBudget = new FileTraversalBudget({
    limits: ctx.limits,
    signal: ctx.signal,
    executionScope: ctx.executionScope,
    site: "ls",
}), traversalDepth = 0, ancestorIdentities = new Set(), visitAlreadyCharged = false) {
    ...
    try {
        ...
        entries = await sortNames(entries, (entry) => (fullPath === "/" ? `/${entry}` : `${fullPath}/${entry}`), ctx, sortKey, false, traversalBudget);
        ...
        if (longFormat) {
            ...
            for (const entry of specialEntries) {
                stdout = appendLsOutput(ctx, stdout, `drwxr-xr-x 1 user user     0 Jan  1 00:00 ${entry}\n`);
            }
        ...
        if (recursive) {
            ...
            const dirOrder = await sortNames(dirEntries.map((d) => d.name), (name) => (fullPath === "/" ? `/${name}` : `${fullPath}/${name}`), ctx, sortKey, reverse, traversalBudget);
            const dirRank = new Map(dirOrder.map((name, index) => [name, index]));
            dirEntries.sort((a, b) => (dirRank.get(a.name) ?? 0) - (dirRank.get(b.name) ?? 0));
            ...
            for (let i = 0; i < dirEntries.length; i += DEFAULT_BATCH_SIZE) {
                ...
                const batchResults = await Promise.all(batch.map(async (dir) => {
                    ...
                    const result = await listPath(subPath, ctx, showAll, showAlmostAll, longFormat, recursive, false, reverse, humanReadable, sortKey, classifyFiles, true, traversalBudget, traversalDepth + 1, childAncestors);
            ...
            subResults.sort((a, b) => (dirRank.get(a.name) ?? 0) - (dirRank.get(b.name) ?? 0));
            // Append results
