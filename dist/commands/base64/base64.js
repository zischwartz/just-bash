...
function assertWithinLimit(value, limit, label, limitType) {
    if (value > limit) {
        throw new ExecutionLimitError(`base64: ${label} limit exceeded (${limit} bytes)`, limitType);
    }
}
...
function decodedLengthUpperBound(input) {
    if (input.length === 0)
        return 0;
...
async function readBinary(ctx, files, cmdName) {
    const maxInputSize = Math.min(ctx.limits.maxInputBytes, ctx.limits.maxStringLength);
    const stdinBinary = latin1FromBytes(ctx.stdin);
    // No files - read from stdin
    if (files.length === 0 || (files.length === 1 && files[0] === "-")) {
        assertWithinLimit(stdinBinary.length, maxInputSize, "input size", "string_length");
    ...
    for (const file of files) {
        if (file === "-") {
            assertWithinLimit(totalLength + stdinBinary.length, maxInputSize, "input size", "string_length");
            // Convert binary string directly to bytes without UTF-8 re-encoding
        ...
        try {
            ...
            assertWithinLimit(totalLength + stat.size, maxInputSize, "input size", "string_length");
            const data = await ctx.fs.readFileBuffer(filePath);
            assertWithinLimit(totalLength + data.length, maxInputSize, "input size", "string_length");
        ...
        try {
            if (decode) {
                ...
                if (typeof Buffer !== "undefined") {
                    ...
                    assertWithinLimit(estimatedLength, ctx.limits.maxStringLength, "decoded size", "string_length");
                    assertWithinLimit(estimatedLength, ctx.limits.maxOutputSize, "output size", "output_size");
                    const decoded = Buffer.from(cleaned, "base64");
                    assertWithinLimit(decoded.length, ctx.limits.maxOutputSize, "output size", "output_size");
            ...
            const newlineCount = wrapCols > 0 && encodedLength > 0
                ? Math.ceil(encodedLength / wrapCols)
                : 0;
            if (newlineCount > ctx.limits.maxArrayElements) {
                throw new ExecutionLimitError(`base64: wrapped line limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
            }
            assertWithinLimit(encodedLength, ctx.limits.maxStringLength, "encoded size", "string_length");
            assertWithinLimit(encodedLength + newlineCount, ctx.limits.maxOutputSize, "output size", "output_size");
            // Use Buffer if available (Node.js) for better large file handling
            let encoded;
            if (typeof Buffer !== "undefined") {
                const buffer = Buffer.from(readResult.data);
