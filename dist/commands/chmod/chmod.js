...
export const chmodCommand = {
    ...
    async execute(args, ctx) {
        ...
        const output = new BoundedStringBuilder(Math.min(ctx.limits.maxOutputSize, ctx.limits.maxStringLength), "chmod");
        let stderr = "";
        let anyError = false;
        for (const file of files) {
            ...
            try {
                ...
                if (recursive) {
                    ...
                    const stat = await ctx.fs.stat(filePath);
                    if (stat.isDirectory) {
                        await chmodRecursive(ctx, filePath, isNumericMode ? numericMode : undefined, isNumericMode ? undefined : modeArg, verbose, output);
                    }
                }
            }
            catch (error) {
...
async function chmodRecursive(ctx, dir, numericMode, symbolicMode, verbose, output) {
    ...
    await traverseFileTree({
        ...
        if (entry.depth === 0 || entry.phase !== "enter")
            return;
        const modeValue = numericMode !== undefined
            ? numericMode
        ...
        if (verbose) {
            output.append(`mode of '${entry.path}' changed to ${modeValue.toString(8).padStart(4, "0")}\n`);
        }
    });
}
/**
