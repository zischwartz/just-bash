...
function resolveCreationLimits(options) {
    return {
        ...
        maxCompressedSize: positiveLimit(options?.maxCompressedSize, options?.maxArchiveSize ?? MAX_ARCHIVE_SIZE),
        ...
        allowTrustedWholeBufferCodecs: options?.allowTrustedWholeBufferCodecs ?? true,
    };
}
function validateCreationBudget(entries, limits) {
    if (entries.length > limits.maxEntries) {
        throw new Error(`Too many archive entries (max ${limits.maxEntries})`);
    }
    const addEstimated = (bytes) => {
    ...
    for (const entry of entries) {
        ...
        const contentSize = entry.isDirectory || entry.isSymlink
            ? 0
            : typeof entry.content === "string"
                ? utf8ByteLength(entry.content)
                : (entry.content?.length ?? 0);
        if (contentSize > limits.maxEntrySize) {
            throw new Error(`Archive entry too large (max ${limits.maxEntrySize} bytes)`);
        }
        const normalizedName = entry.isDirectory && !entry.name.endsWith("/")
            ? `${entry.name}/`
            : entry.name;
        const nameBytes = utf8ByteLength(normalizedName);
        const linkBytes = utf8ByteLength(entry.linkTarget ?? "");
        if (nameBytes > limits.maxArchiveSize ||
            linkBytes > limits.maxArchiveSize) {
            throw new Error(`Archive too large (max ${limits.maxArchiveSize} bytes)`);
        }
        let paxPayloadBytes = 0;
        if (nameBytes > 100)
            paxPayloadBytes += nameBytes + 64;
        if (linkBytes > 100)
            paxPayloadBytes += linkBytes + 64;
        if (!Number.isSafeInteger(paxPayloadBytes)) {
            throw new Error(`Archive too large (max ${limits.maxArchiveSize} bytes)`);
        }
        const paddedContentSize = Math.ceil(contentSize / TAR_BLOCK_SIZE) * TAR_BLOCK_SIZE;
        addEstimated(TAR_BLOCK_SIZE + paddedContentSize);
        if (paxPayloadBytes > 0) {
            const paddedPaxSize = Math.ceil(paxPayloadBytes / TAR_BLOCK_SIZE) * TAR_BLOCK_SIZE;
...
function reserveArchiveBytes(limits, kind, bytes) {
...
function validateTarStructureBudget(data, limits) {
    let offset = 0;
    let entries = 0;
 ...
 */
export async function createArchive(entries, options) {
    const limits = resolveCreationLimits(options);
    const estimatedBytes = validateCreationBudget(entries, limits);
    const lease = reserveArchiveBytes(limits, "archive construction", estimatedBytes);
    try {
 ...
 */
export async function createCompressedArchive(entries, options) {
    const limits = resolveCreationLimits(options);
    const tarBuffer = await createArchive(entries, limits);
    const inputLease = reserveArchiveBytes(limits, "gzip input", tarBuffer.byteLength);
    ...
    try {
        while (true) {
            ...
            try {
                ...
                const chunkLease = reserveArchiveBytes(limits, "gzip chunks", value.byteLength);
                if (chunkLease)
                    chunkLeases.push(chunkLease);
            }
            catch (error) {
                await reader.cancel();
                throw error;
        ...
        for (const lease of chunkLeases)
            lease.release();
        inputLease?.release();
    }
}
 ...
 */
export async function parseArchive(data, options) {
    const limits = resolveCreationLimits(options);
    ...
    if (structureError)
        return { entries: [], error: structureError };
    try {
        ...
        for (const entry of modernEntries) {
            ...
            if (entry.header.size > limits.maxEntrySize ||
 ...
 */
export async function parseCompressedArchive(data, options) {
    const limits = resolveCreationLimits(options);
 ...
 */
async function decompressBzip2(data, limits) {
 ...
 */
async function compressBzip2(data, limits) {
    const budget = new CodecBudget({
        maxInputBytes: limits.maxArchiveSize,
        maxOutputBytes: limits.maxCompressedSize,
        signal: limits.signal,
    ...
    budget.acceptInput(data.length);
    const result = await bzip2Compress(data, 9, limits.maxArchiveSize, limits.maxCompressedSize);
 ...
 */
export async function createBzip2CompressedArchive(entries, options) {
    const limits = resolveCreationLimits(options);
    const tarBuffer = await createArchive(entries, limits);
...
export async function createXzCompressedArchive(entries, options) {
    const limits = resolveCreationLimits(options);
    if (options?.allowNativeCodecs === false ||
        !limits.allowTrustedWholeBufferCodecs) {
        throw new Error("xz compression is disabled by configuration. Enable native and trusted whole-buffer codecs to use it.");
    }
    const tarBuffer = await createArchive(entries, limits);
    const compressed = await compressXz(tarBuffer);
    limits.signal?.throwIfAborted();
    if (compressed.length > limits.maxCompressedSize) {
        throw new Error(`xz archive: output exceeds limit (${limits.maxCompressedSize} bytes)`);
    }
    return compressed;
}
/**
 ...
 */
export async function parseBzip2CompressedArchive(data, options) {
...
export async function parseXzCompressedArchive(data, options) {
    ...
    if (options?.allowNativeCodecs === false ||
        ...
        return {
            entries: [],
            error: "xz decompression is disabled by configuration. Enable native and trusted whole-buffer codecs, or decompress the archive externally before extraction.",
        };
    }
    if (data.length > limits.maxCompressedSize) {
        return {
...
export async function createZstdCompressedArchive(entries, options) {
    ...
    if (options?.allowNativeCodecs === false ||
        !limits.allowTrustedWholeBufferCodecs) {
        throw new Error("zstd compression is disabled by configuration. Enable native and trusted whole-buffer codecs to use it.");
    }
    const tarBuffer = await createArchive(entries, limits);
    const compressed = await compressZstd(tarBuffer);
    limits.signal?.throwIfAborted();
    if (compressed.length > limits.maxCompressedSize) {
        throw new Error(`zstd archive: output exceeds limit (${limits.maxCompressedSize} bytes)`);
    }
    return compressed;
}
/**
...
export async function parseZstdCompressedArchive(data, options) {
    ...
    if (options?.allowNativeCodecs === false ||
        ...
        return {
            entries: [],
            error: "zstd decompression is disabled by configuration. Enable native and trusted whole-buffer codecs, or decompress the archive externally before extraction.",
        };
    }
    if (data.length > limits.maxCompressedSize) {
        return {
