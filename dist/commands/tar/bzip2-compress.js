...
class BitWriter {
    ...
    current = 0;
    bitCount = 0;
    constructor(maxOutputBytes = Number.MAX_SAFE_INTEGER) {
    ...
    pushByte(value) {
        if (this.buffer.length >= this.maxOutputBytes) {
            throw new Error(`bzip2 output exceeds limit (${this.maxOutputBytes} bytes)`);
        }
 ...
 */
export function bzip2Compress(data, blockSizeLevel = 9, maxSize = DEFAULT_MAX_COMPRESS_SIZE, maxOutputSize = Number.MAX_SAFE_INTEGER) {
