...
async function createTarArchive(ctx, options, files) {
    ...
    try {
        if (options.gzip) {
            archiveData = await createCompressedArchive(allEntries, archiveLimits(ctx));
        }
        else if (options.bzip2) {
            archiveData = await createBzip2CompressedArchive(allEntries, archiveLimits(ctx));
        }
        else if (options.xz) {
            archiveData = await createXzCompressedArchive(allEntries, archiveLimits(ctx));
        }
        else if (options.zstd) {
            archiveData = await createZstdCompressedArchive(allEntries, archiveLimits(ctx));
        }
        else {
...
async function extractTarArchive(ctx, options, specificFiles) {
    ...
    if (useGzip) {
        ...
        parseResult = await parseBzip2CompressedArchive(archiveData, archiveLimits(ctx));
    }
    else if (useXz) {
        parseResult = await parseXzCompressedArchive(archiveData, archiveLimits(ctx));
    }
    else if (useZstd) {
        parseResult = await parseZstdCompressedArchive(archiveData, archiveLimits(ctx));
    }
    else {
