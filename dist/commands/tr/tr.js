import { decodeBytesToUtf8, latin1FromBytes, utf8ByteLength, } from "../../encoding.js";
...
function expandRange(set, maxLength, maxIterations, budget) {
    let result = "";
    let i = 0;
    const append = (value) => {
        if (value.length > maxLength - result.length) {
            throw new ExecutionLimitError(`tr: expanded SET exceeds string length limit (${maxLength})`, "string_length");
        }
        result += value;
    };
    const useIterations = (count = 1) => {
        if (count > maxIterations - budget.iterations) {
            throw new ExecutionLimitError(`tr: SET expansion iteration limit exceeded (${maxIterations})`, "iterations");
        }
...
export const trCommand = {
    ...
    async execute(args, ctx) {
        ...
        const maxOutputSize = Math.min(ctx.limits.maxOutputSize, ctx.limits.maxStringLength);
        try {
            const expansionBudget = { iterations: 0 };
            set1Raw = expandRange(sets[0], maxStringLength, maxIterations, expansionBudget);
            set2 =
                sets.length > 1
                    ? expandRange(sets[1], maxStringLength, maxIterations, expansionBudget)
                    : "";
        }
        catch (e) {
        ...
        if (latin1FromBytes(ctx.stdin).length > maxStringLength) {
            throw new ExecutionLimitError(`tr: input size limit exceeded (${maxStringLength} bytes)`, "string_length");
        }
        const content = decodeBytesToUtf8(ctx.stdin);
        if (set1Raw.length > maxArrayElements || set2.length > maxArrayElements) {
            throw new ExecutionLimitError(`tr: array element limit exceeded (${maxArrayElements})`, "array_elements");
        }
        ...
        const appendOutput = (value) => {
            ...
            if (bytes > maxOutputSize - outputBytes) {
                throw new ExecutionLimitError(`tr: output size limit exceeded (${maxOutputSize} bytes)`, "output_size");
            }
            output += value;
            outputBytes += bytes;
        };
