...
function splitExactBounded(input, delimiter, maxItems) {
    ...
    while (start <= input.length) {
        ...
        if (item.length > 0) {
            if (items.length >= maxItems) {
                throw new ExecutionLimitError(`xargs: array element limit exceeded (${maxItems})`, "array_elements");
            }
            items.push(item);
        }
        if (end === -1)
...
function splitWhitespaceBounded(input, maxItems) {
    const items = [];
    let start = -1;
...
export const xargsCommand = {
    ...
    async execute(args, ctx) {
        ...
        const maxStringLength = Math.min(ctx.limits.maxInputBytes, ctx.limits.maxStringLength);
        ...
        if (utf8ByteLength(stdinText) > maxStringLength) {
            throw new ExecutionLimitError(`xargs: input size limit exceeded (${maxStringLength} bytes)`, "string_length");
        }
        if (maxProcs !== null && maxProcs > maxArrayElements) {
            throw new ExecutionLimitError(`xargs: array element limit exceeded (${maxArrayElements})`, "array_elements");
        }
        let items;
        if (nullSeparator) {
        ...
        const appendOutput = (result) => {
            ...
            const addedBytes = utf8ByteLength(result.stdout) + utf8ByteLength(result.stderr);
            if (addedBytes > maxOutputSize - outputBytes) {
                throw new ExecutionLimitError(`xargs: output size limit exceeded (${maxOutputSize} bytes)`, "output_size");
            }
            if (result.stdout)
        ...
        const executeCommand = async (cmdArgs) => {
            if (++commandIterations > maxIterations) {
                throw new ExecutionLimitError(`xargs: iteration limit exceeded (${maxIterations})`, "iterations");
            }
            ...
            for (const value of cmdArgs) {
                if (utf8ByteLength(value) > maxStringLength) {
                    throw new ExecutionLimitError(`xargs: string length limit exceeded (${maxStringLength} bytes)`, "string_length");
                }
            }
            if (verbose) {
        ...
        if (replaceStr !== null) {
            ...
            const replaceBounded = (template, item) => {
                let occurrences = 0;
                let position = 0;
                while (true) {
                ...
                const prospectiveBytes = utf8ByteLength(template) +
            ...
            const cmdArgsList = items.map((item) => command.map((c) => replaceBounded(c, item)));
