 ...
 */
function parseTabStops(spec, maxTabStop, maxStops) {
    const parts = spec.split(",").map((s) => s.trim());
    if (parts.length > maxStops)
        return null;
    const stops = [];
...
function expandLine(line, options, maxOutputBytes) {
        ...
        if (char === "\t") {
            if (leadingOnly && !inLeadingWhitespace) {
                if (outputBytes + 1 > maxOutputBytes) {
                    throw new ExecutionLimitError(`expand: output size limit exceeded (${maxOutputBytes} bytes)`, "output_size");
                }
                result += char;
                outputBytes++;
...
function processContent(content, options, maxOutputBytes) {
    if (content === "") {
        return "";
    }
    ...
    for (const line of lines) {
        ...
        const expanded = expandLine(line, options, maxOutputBytes - outputBytes - newlineBytes);
...
export const expand = {
    ...
    execute: async (args, ctx) => {
        ...
        const files = [];
        const maxTabStop = Math.min(ctx.limits.maxStringLength, ctx.limits.maxOutputSize);
        ...
        if (files.length === 0) {
            ...
            for (const file of files) {
                ...
                output += processContent(content, options, maxTabStop - utf8ByteLength(output));
            }
        }
