...
export async function processHeadTailFiles(ctx, options, cmdName, contentProcessor) {
    ...
    if (files.length > ctx.limits.maxArrayElements) {
        throw new ExecutionLimitError(`${cmdName}: file operand limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
    }
    ...
    const maxBytes = Math.min(ctx.limits.maxOutputSize, ctx.limits.maxStringLength);
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        try {
            ...
            if (stat.size > ctx.limits.maxInputBytes - aggregateInput) {
                throw new ExecutionLimitError(`${cmdName}: aggregate input size limit exceeded (${ctx.limits.maxInputBytes} bytes)`, "string_length");
            }
