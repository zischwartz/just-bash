...
export class AwkParser {
    ...
    parse(input) {
        if (input.length > this.limits.maxSourceLength) {
            throw new ExecutionLimitError(`awk: source length limit exceeded (${this.limits.maxSourceLength})`, "string_length");
        }
        ...
        for (const token of this.tokens) {
            ...
            if (token.type === TokenType.LPAREN ||
                ...
                if (delimiterDepth > this.limits.maxDepth) {
                    throw new ExecutionLimitError(`awk: parser depth limit exceeded (${this.limits.maxDepth})`, "recursion");
                }
            }
    ...
    useOperation(count = 1) {
        if (count > this.limits.maxOperations - this.operations) {
            throw new ExecutionLimitError(`awk: parser operation limit exceeded (${this.limits.maxOperations})`, "iterations");
        }
    ...
    assertArrayLength(length) {
        if (length >= this.limits.maxTokens) {
            throw new ExecutionLimitError(`awk: AST array limit exceeded (${this.limits.maxTokens})`, "array_elements");
        }
    }
    match(...types) {
