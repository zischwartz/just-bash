...
function createAwkStringBuilder(maxBytes) {
    return new BoundedStringBuilder(maxBytes, "awk string", () => new ExecutionLimitError(`string size limit exceeded (${maxBytes} bytes)`, "string_length"));
}
function awkStringLimit(ctx) {
    return ctx.maxOutputSize > 0 ? ctx.maxOutputSize : DEFAULT_AWK_STRING_LIMIT;
}
function boundedRegexReplace(target, regex, shouldReplace, replacer, maxBytes) {
...
async function awkSplit(args, ctx, evaluator) {
    ...
    const available = ctx.maxArrayElements - ctx.arrayElementCount + previousCount;
...
async function awkSub(args, ctx, evaluator) {
    ...
    try {
        ...
        const replaced = boundedRegexReplace(target, regex, (matchNumber) => matchNumber === 1, (match) => createSubReplacement(replacement, match[0], awkStringLimit(ctx)), awkStringLimit(ctx));
...
async function awkGsub(args, ctx, evaluator) {
    ...
    try {
        const regex = createUserRegex(pattern, "g");
        const replaced = boundedRegexReplace(target, regex, () => true, (match) => createSubReplacement(replacement, match[0], awkStringLimit(ctx)), awkStringLimit(ctx));
        applyTargetValue(targetName, replaced.value, ctx);
        return replaced.replacements;
    }
    catch (error) {
...
function createSubReplacement(replacement, match, maxBytes) {
...
async function awkGensub(args, ctx, evaluator) {
    ...
    try {
        ...
        if (isGlobal) {
            const regex = createUserRegex(pattern, "g");
            return boundedRegexReplace(target, regex, () => true, (match) => processGensub(replacement, match[0], match.slice(1), awkStringLimit(ctx)), awkStringLimit(ctx)).value;
        }
        else {
            const regex = createUserRegex(pattern, "g");
            return boundedRegexReplace(target, regex, (matchNumber) => matchNumber === occurrenceNum, (match) => processGensub(replacement, match[0], match.slice(1), awkStringLimit(ctx)), awkStringLimit(ctx)).value;
        }
    }
    catch (error) {
        rethrowFatalExecutionError(error);
...
function processGensub(replacement, match, groups, maxBytes) {
    const result = createAwkStringBuilder(maxBytes);
    let i = 0;
    while (i < replacement.length) {
        if (replacement[i] === "\\" && i + 1 < replacement.length) {
...
async function awkSprintf(args, ctx, evaluator) {
    if (args.length === 0)
        return "";
...
export function formatPrintf(format, values, maxBytes = DEFAULT_AWK_STRING_LIMIT) {
    let valueIdx = 0;
    let result = "";
    let resultBytes = 0;
    let i = 0;
    ...
    const append = (value) => {
        ...
        if (bytes > maxBytes - resultBytes) {
            throw new ExecutionLimitError(`formatted string size limit exceeded (${maxBytes} bytes)`, "string_length");
        }
        result += value;
        resultBytes += bytes;
    };
        ...
        if (format[i] === "%" && i + 1 < format.length) {
            ...
            if (format[j] === "*") {
                ...
                if (!Number.isFinite(w) || !Number.isSafeInteger(w)) {
                    throw new ExecutionLimitError(`printf width limit exceeded (${maxFieldWidth} bytes)`, "string_length");
                }
                if (w < 0) {
                    flags += "-";
                    width = String(-w);
            ...
            if (format[j] === ".") {
                ...
                if (format[j] === "*") {
                    ...
                    const parsedPrecision = precVal !== undefined ? Math.floor(Number(precVal)) : 0;
                    if (!Number.isFinite(parsedPrecision) ||
                        !Number.isSafeInteger(parsedPrecision)) {
                        throw new ExecutionLimitError(`printf precision limit exceeded (${maxFieldWidth} bytes)`, "string_length");
                    }
            ...
            switch (spec) {
                ...
                case "f": {
                    ...
                    if (prec > 100) {
                        throw new ExecutionLimitError("printf floating-point precision limit exceeded (100 digits)", "string_length");
                    }
