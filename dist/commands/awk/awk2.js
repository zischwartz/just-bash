...
export const awkCommand2 = {
    ...
    async execute(args, ctx) {
        ...
        const runtimeCtx = createRuntimeContext({
            ...
            maxOutputSize: Math.min(ctx.limits.maxStringLength, ctx.limits.maxOutputSize),
        ...
        try {
            ...
            const processInput = async (filename, content) => {
                const contentBytes = utf8ByteLength(content);
                if (contentBytes > maxInputBytes - aggregateInputBytes) {
                    throw new ExecutionLimitError(`aggregate input size limit exceeded (${maxInputBytes} bytes)`, "string_length");
                }
                ...
                for (let index = 0; index < content.length; index++) {
                    ...
                    if (recordCount > maxRecords + 1) {
                        throw new ExecutionLimitError(`record array limit exceeded (${maxRecords})`, "array_elements");
                    }
                }
                ...
                while (runtimeCtx.lineIndex < lines.length - 1) {
                    ...
                    await withDefenseContext("line execution", () => interp.executeLine(lines[activeLineIndex]));
            ...
            if (files.length > 0) {
                for (const file of files) {
                    ...
                    try {
                        ...
                        const stat = await withDefenseContext("input file stat", () => ctx.fs.stat(filePath));
                        if (stat.size > maxInputBytes - aggregateInputBytes) {
                            throw new ExecutionLimitError(`aggregate input size limit exceeded (${maxInputBytes} bytes)`, "string_length");
                        }
                        content = await withDefenseContext("input file read", () => ctx.fs.readFile(filePath));
                    }
                    catch (e) {
                        if (e instanceof SecurityViolationError ||
                    ...
                    await withDefenseContext("input processing", () => processInput(file, content));
                    if (runtimeCtx.shouldExit)
                        break;
                }
            }
                ...
                await withDefenseContext("stdin processing", () => processInput("", decodeBytesToUtf8(ctx.stdin)));
            }
