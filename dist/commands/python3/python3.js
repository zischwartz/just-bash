...
function processNextExecution(queueState) {
    ...
    if (queueState.activeExecution !== null ||
    ...
    const settleActiveExecution = async (result, terminateWorker) => {
    ...
    const onMessage = bindDefenseContextCallback(next.requireDefenseContext, "python3", "worker message callback", (msg) => {
        ...
        void settleActiveExecution(normalizeWorkerMessage(msg, next.input.protocolToken), true);
    });
    ...
    const onExit = bindDefenseContextCallback(next.requireDefenseContext, "python3", "worker exit callback", () => {
        void settleActiveExecution({ success: false, error: "Worker exited unexpectedly" }, false);
    });
 ...
 */
async function executePython(pythonCode, stdin, ctx, scriptPath, scriptArgs = []) {
    const sharedBuffer = createSharedBuffer();
    const bridgeHandler = new BridgeHandler(sharedBuffer, ctx.fs, ctx.cwd, "python3", ctx.fetch, ctx.limits.maxOutputSize);
    ...
    controller.assertMessageSize({ ...workerInput, sharedBuffer: undefined }, "request");
    const workerPromise = new Promise((resolve) => {
        ...
        const onCancel = bindDefenseContextCallback(ctx.requireDefenseContext, "python3", "worker cancellation callback", (reason) => {
            bridgeHandler.stop?.();
            queueEntry.cancel({
                success: false,
                error: reason === "abort"
    ...
    if (!workerResult.success && workerResult.error) {
        ...
        const canceled = workerResult.error === controller.timeoutMessage() ||
