...
async function odExecute(args, ctx) {
    ...
    // Build output lines
    const output = new BoundedStringBuilder(Math.min(ctx.limits.maxOutputSize, ctx.limits.maxStringLength), "od");
    const formatCount = outputFormats.length;
    if (formatCount > 0 &&
        inputBytes.length > Math.floor(ctx.limits.maxLoopIterations / formatCount)) {
        throw new ExecutionLimitError(`od: format work limit exceeded (${ctx.limits.maxLoopIterations})`, "iterations");
    }
    ctx.executionScope?.consumeWork(inputBytes.length * formatCount, "od formatting");
    for (let offset = 0; offset < inputBytes.length; offset += bytesPerLine) {
        ...
        for (let formatIdx = 0; formatIdx < outputFormats.length; formatIdx++) {
            ...
            let formatted;
            if (format === "char") {
                formatted = Array.from(inputBytes.slice(offset, offset + bytesPerLine), (char) => formatCharByte(char.charCodeAt(0)));
            }
            else if (format === "hex") {
                formatted = Array.from(inputBytes.slice(offset, offset + bytesPerLine), (char) => formatHexByte(char.charCodeAt(0)));
            }
            else {
                formatted = Array.from(inputBytes.slice(offset, offset + bytesPerLine), (char) => formatOctalByte(char.charCodeAt(0)));
            }
