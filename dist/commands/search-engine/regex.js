 ...
 */
function extractPreFilter(jsPattern, ignoreCase) {
    let core = jsPattern;
 ...
 */
function literalFromAlternative(alt) {
    ...
    if (inner.endsWith("$")) {
        let bs = 0;
        for (let i = inner.length - 2; i >= 0 && inner[i] === "\\"; i--)
            bs++;
        if (bs % 2 === 0) {
    ...
    for (let i = 0; i < inner.length; i++) {
        ...
        if (c === "\\") {
            ...
            if (/[dDwWsSbBAZzGQE0-9ckpPNXRxuU]/.test(next))
                return null;
