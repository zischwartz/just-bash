 ...
 */
function splitFields(line, separator, noMerge, limit) {
    if (separator) {
        if (noMerge) {
 ...
 */
function formatTable(rows, outputSep, maxOutputBytes) {
    if (rows.length === 0)
        return "";
    const widths = calculateColumnWidths(rows);
    const output = new BoundedStringBuilder(maxOutputBytes, "column", undefined, 1);
    for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
        ...
        for (let i = 0; i < row.length; i++) {
            if (i > 0)
                output.append(outputSep);
            output.append(row[i]);
            if (i < row.length - 1)
                output.repeat(" ", widths[i] - row[i].length);
        }
    }
    return output.build();
}
 ...
 */
function formatFill(items, width, outputSep, maxOutputBytes) {
    if (items.length === 0)
        return "";
...
export const column = {
    ...
    execute: async (args, ctx) => {
            ...
            for (const file of files) {
                if (file === "-") {
                    ...
                    if (inputBytes > maxInputBytes) {
                        throw new ExecutionLimitError(`column: input size limit exceeded (${maxInputBytes} bytes)`, "string_length");
                    }
                    parts.push(part);
                }
                else {
        ...
        if (lines.length > maxArrayElements) {
            throw new ExecutionLimitError(`column: field limit exceeded (${maxArrayElements})`, "array_elements");
        }
