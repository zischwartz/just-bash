...
const FUNCTION_MAP = {
    ...
    coalesce: (args) => {
        ...
        for (let i = args.length - 2; i >= 0; i--) {
            ...
            const condition = makeBinaryOp("and", makeBinaryOp("!=", current, { type: "Literal", value: null }), makeBinaryOp("!=", current, { type: "Literal", value: "" }));
 ...
 * Transform moonblade AST to jq AST
 */
export function moonbladeToJq(expr, rowContext = true, limits = {}) {
    const budget = {
        nodes: 0,
        operations: 0,
...
function convertMoonblade(expr, rowContext, budget, depth) {
    if (depth >= budget.maxDepth) {
        throw new ExecutionLimitError(`xan: expression conversion depth limit exceeded (${budget.maxDepth})`, "recursion");
    }
    if (++budget.nodes > budget.maxNodes) {
        throw new ExecutionLimitError(`xan: expression conversion AST limit exceeded (${budget.maxNodes})`, "array_elements");
    }
    if (++budget.operations > budget.maxOperations) {
        throw new ExecutionLimitError(`xan: expression conversion operation limit exceeded (${budget.maxOperations})`, "iterations");
    }
    const convert = (child) => convertMoonblade(child, rowContext, budget, depth + 1);
    switch (expr.type) {
        case "int":
        case "float":
