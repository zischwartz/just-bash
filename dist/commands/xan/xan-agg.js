 ...
 */
import { buildAggRow, computeAgg, estimateSortingWork, parseAggExpr, } from "./aggregation.js";
import { createSafeRow, DerivedCsvBudget, formatCsv, readCsvInput, safeSetRow, } from "./csv.js";
function getAggregationLimits(ctx, budget) {
    return {
...
function frequencyResultCount(counts, noExtra, limit) {
...
export async function cmdGroupby(args, ctx) {
    ...
    for (const key of groupOrder) {
        ...
        for (const spec of specs) {
            safeSetRow(row, spec.alias, computeAgg(groupData, spec, evalOptions, aggregationLimits));
        }
        results.push(row);
    }
    ...
    if (groupCol) {
        ...
        for (const col of targetCols) {
            for (const [groupKey, groupData] of groups) {
                ...
                budget.addRows(frequencyResultCount(counts, noExtra, limit), resultHeaders.length);
...
export async function cmdStats(args, ctx) {
    ...
    for (const col of targetCols) {
        ...
        safeSetRow(result, "mean", isNumeric ? Math.round((sum / numericCount) * 1e10) / 1e10 : "");
        results.push(result);
    }
    return {
