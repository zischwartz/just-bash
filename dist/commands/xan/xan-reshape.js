...
import { createSafeRow, DerivedCsvBudget, formatCsv, readCsvInput, safeSetRow, toSafeRow, } from "./csv.js";
/**
    ...
    for (const row of data) {
        ...
        if (strValue === "") {
            ...
            while (start <= strValue.length) {
                resultBudget.addRow(newHeaders.length);
                const next = separator === "" ? start + 1 : strValue.indexOf(separator, start);
    ...
    for (const row of data) {
        ...
        if (key !== currentKey) {
            ...
            if (currentRow !== null) {
                ...
                safeSetRow(newRow, targetCol, boundedJoin(currentValues, separator, Math.min(ctx.limits.maxStringLength, ctx.limits.maxOutputSize), "xan implode"));
                resultBudget.addRow(newHeaders.length);
                newData.push(newRow);
            }
            // Start new group
...
export async function cmdJoin(args, ctx) {
    ...
    if (checkedAdd(headers1.length, headers2Unique.length, "xan join") >
        ctx.limits.maxArrayElements) {
        throw new ExecutionLimitError(`xan join: output column limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
    }
    ...
    for (const row1 of data1) {
        ...
        prospectiveRows = checkedAdd(prospectiveRows, matches?.length ?? (joinType === "left" || joinType === "full" ? 1 : 0), "xan join");
    }
...
export async function cmdPivot(args, ctx) {
    ...
    for (const row of data) {
        ...
        if (!pivotValueSet.has(val)) {
            if (pivotValues.length >= ctx.limits.maxArrayElements) {
                throw new ExecutionLimitError(`xan pivot: pivot value limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
            }
    ...
    // Build output
    const collidingHeader = pivotValues.find((value) => groupCols.includes(value));
    ...
    const outputColumnCount = checkedAdd(groupCols.length, pivotValues.length, "xan pivot");
    if (outputColumnCount > ctx.limits.maxArrayElements) {
        throw new ExecutionLimitError(`xan pivot: output column limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
    }
...
export async function cmdMerge(args, ctx) {
    ...
    const mergedRows = allData.reduce((count, entry) => checkedAdd(count, entry.data.length, "xan merge"), 0);
