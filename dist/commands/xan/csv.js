...
import { BoundedStringBuilder, checkedAdd, checkedMultiply, } from "../../bounded-builder.js";
...
export class DerivedCsvBudget {
    ...
    addRows(rowCount, cellCount) {
        ...
        if (!Number.isSafeInteger(rowCount) ||
            ...
            throw new ExecutionLimitError(`${this.site}: derived CSV result limit exceeded`, "array_elements");
        }
        this.consumeWork(checkedMultiply(rowCount, checkedAdd(1, cellCount, this.site), this.site));
        this.rows += rowCount;
        this.cells += cells;
    }
}
...
export function parseCsv(input, limits = {}) {
    ...
    if (utf8ByteLength(input) > maxStringLength) {
        throw new ExecutionLimitError(`xan: CSV input size limit exceeded (${maxStringLength} bytes)`, "string_length");
    }
    const safeData = [];
    const normalizedInput = input.trim();
    const headerResult = Papa.parse(normalizedInput, {
        preview: 1,
        skipEmptyLines: true,
    });
    let headers = headerResult.data[0] ?? [];
    if (headers.length > maxArrayElements) {
        throw new ExecutionLimitError(`xan: CSV column limit exceeded (${maxArrayElements})`, "array_elements");
    }
    let cellCount = 0;
    let limitError;
    Papa.parse(normalizedInput, {
        header: true,
        dynamicTyping: true,
        skipEmptyLines: true,
        step: (result, parser) => {
            ...
            if (safeData.length === 0 && result.meta.fields) {
                if (result.meta.fields.length > maxArrayElements) {
                    limitError = new ExecutionLimitError(`xan: CSV column limit exceeded (${maxArrayElements})`, "array_elements");
                    parser.abort();
                    return;
                }
            ...
            if (safeData.length >= maxRows || fields.length > maxCells - cellCount) {
                limitError = new ExecutionLimitError(safeData.length >= maxRows
                    ? `xan: CSV row limit exceeded (${maxRows})`
                    : `xan: CSV cell limit exceeded (${maxCells})`, "array_elements");
                parser.abort();
                return;
            }
...
export function parseCsvRows(input, limits = {}) {
    const maxStringLength = limits.maxStringLength ?? 10 * 1024 * 1024;
    const maxArrayElements = limits.maxArrayElements ?? 100_000;
    const maxRows = limits.maxRows ?? maxArrayElements;
    const maxCells = limits.maxCells ?? 10_000_000;
    ...
    Papa.parse(input.trim(), {
        ...
        step: (result, parser) => {
            ...
            if (rows.length >= maxRows ||
                ...
                limitError = new ExecutionLimitError(rows.length >= maxRows
                    ? `xan: CSV row limit exceeded (${maxRows})`
                    : row.length > maxArrayElements
...
export function formatCsv(headers, data, ctx) {
    return formatCsvValues(data.length, headers.length, (row, column) => data[row][headers[column]], ctx, headers);
}
/** Format object rows in header order, omitting the header record itself. */
export function formatCsvWithoutHeaders(headers, data, ctx) {
    return formatCsvValues(data.length, headers.length, (row, column) => data[row][headers[column]], ctx);
}
/** Format already-materialized cells without Papa.unparse's unbounded copy. */
export function formatCsvRows(data, ctx, headers) {
    return formatCsvValues(data.length, undefined, (row, column) => data[row][column], ctx, headers, (row) => data[row].length);
}
function formatCsvValues(rowCount, columnCount, valueAt, ctx, headers, columnsAt = () => columnCount ?? 0) {
    const maxBytes = ctx?.limits.maxOutputSize ?? Number.MAX_SAFE_INTEGER;
    const output = new BoundedStringBuilder(maxBytes, "xan CSV output", ctx
        ? () => new ExecutionLimitError(`xan: output size limit exceeded (${maxBytes} bytes)`, "output_size")
        : undefined);
    const appendCell = (value) => {
        const raw = value === null || value === undefined ? "" : String(value);
        let quoteCount = 0;
        for (let index = 0; index < raw.length; index++) {
            if (raw.charCodeAt(index) === 34)
                quoteCount++;
        }
        const needsQuotes = quoteCount > 0 ||
            raw.includes("\r") ||
            raw.includes("\n") ||
            raw.includes(",") ||
            raw.includes("\uFEFF") ||
        ...
        output.reserve(checkedAdd(utf8ByteLength(raw), checkedAdd(quoteCount, needsQuotes ? 2 : 0, "xan CSV output"), "xan CSV output"));
        const escaped = quoteCount > 0 ? raw.replace(/"/g, '""') : raw;
        output.append(needsQuotes ? `"${escaped}"` : escaped);
    };
    const appendRow = (row, columns) => {
...
export async function readCsvInput(args, ctx) {
    ...
    const { headers, data } = parseCsv(input, {
        maxStringLength: Math.min(ctx.limits.maxInputBytes, ctx.limits.maxStringLength),
