...
export class Tokenizer {
    ...
    tokenize() {
        if (this.input.length > this.maxSourceLength) {
            throw new ExecutionLimitError(`xan: expression length limit exceeded (${this.maxSourceLength})`, "string_length");
        }
    ...
    assertTokenCapacity() {
        if (this.tokens.length >= this.maxTokens) {
            throw new ExecutionLimitError(`xan: expression token limit exceeded (${this.maxTokens})`, "array_elements");
        }
    }
    skipWhitespace() {
        while (this.pos < this.input.length) {
