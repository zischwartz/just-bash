...
import { createSafeRow, DerivedCsvBudget, formatCsv, formatCsvRows, parseCsvRows, readCsvInput, safeSetRow, } from "./csv.js";
/**
...
export async function cmdFixlengths(args, ctx) {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if ((arg === "-l" || arg === "--length") && i + 1 < args.length) {
            ...
            if (!Number.isSafeInteger(targetLen) ||
                targetLen > ctx.limits.maxArrayElements) {
                throw new ExecutionLimitError(`xan fixlengths: column limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
            }
        }
...
export async function cmdSplit(args, ctx) {
    ...
    if (partCount > ctx.limits.maxArrayElements) {
        throw new ExecutionLimitError(`xan split: output part limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
    }
    ...
    try {
        ...
        for (let i = 0; i < partCount; i++) {
            ...
            await ctx.fs.writeFile(filePath, formatCsv(headers, data.slice(start, end), ctx));
        }
        // xan emits text; the pipeline handles encoding.
        return {
        ...
        const output = new BoundedStringBuilder(ctx.limits.maxOutputSize, "xan split");
        for (let i = 0; i < partCount; i++) {
            const start = i * chunkSize;
            const end = Math.min(start + chunkSize, data.length);
...
export async function cmdPartition(args, ctx) {
    ...
    try {
        ...
        const output = new BoundedStringBuilder(ctx.limits.maxOutputSize, "xan partition");
        for (const [val, rows] of groups) {
...
async function cmdToJson(args, ctx) {
    ...
    budget.addRows(data.length, data.length > 0 ? Object.keys(data[0]).length : 0);
    if (ctx.limits.maxOutputSize < 1) {
        throw new ExecutionLimitError("xan to json: output size limit exceeded (0 bytes)", "output_size");
    }
    const maxBytes = ctx.limits.maxOutputSize - 1;
    const output = formatJsonValue(data, maxBytes);
    // xan emits text; the pipeline handles encoding.
    return {
        stdout: `${output}\n`,
        stderr: "",
...
async function cmdFromJson(fileArgs, ctx) {
    ...
    try {
        const maxInputBytes = Math.min(ctx.limits.maxInputBytes, ctx.limits.maxStringLength);
        if (utf8ByteLength(input) > maxInputBytes) {
            throw new ExecutionLimitError(`xan from: input size limit exceeded (${maxInputBytes} bytes)`, "string_length");
        }
        ...
        if (Array.isArray(data[0])) {
            ...
            if (headers.length > ctx.limits.maxArrayElements) {
                throw new ExecutionLimitError(`xan from: column limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
            }
            ...
            for (let rowIndex = 1; rowIndex < arrays.length; rowIndex++) {
                ...
                for (let i = 0; i < headerNames.length; i++) {
                    safeSetRow(obj, headerNames[i], row[i]);
                }
                csvData.push(obj);
            }
            // xan emits text; the pipeline handles encoding.
