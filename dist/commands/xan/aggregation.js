...
export function estimateSortingWork(length) {
    if (length < 2)
        return 0;
 ...
 */
export function parseAggExpr(expr, limits = {}) {
    ...
    if (utf8ByteLength(expr) > maxStringLength) {
        throw new ExecutionLimitError(`xan: aggregation expression length limit exceeded (${maxStringLength} bytes)`, "string_length");
    }
    const specs = [];
    let i = 0;
    let operations = 0;
    const useOperation = () => {
        ...
        if (++operations > maxIterations) {
            throw new ExecutionLimitError(`xan: aggregation parser operation limit exceeded (${maxIterations})`, "iterations");
        }
    };
    while (i < expr.length) {
        useOperation();
        ...
        while (i < expr.length && parenDepth > 0) {
            ...
            if (parenDepth > maxDepth) {
                throw new ExecutionLimitError(`xan: aggregation parser depth limit exceeded (${maxDepth})`, "recursion");
            }
            if (parenDepth > 0)
                i++;
        }
        ...
        if (specs.length >= maxArrayElements) {
            throw new ExecutionLimitError(`xan: aggregation specification limit exceeded (${maxArrayElements})`, "array_elements");
        }
...
function evalExpr(row, ast, evalOptions) {
    const results = evaluate(row, ast, evalOptions);
    return results.length > 0 ? results[0] : null;
}
/** Compute aggregation on data */
export function computeAgg(data, spec, evalOptions = {}, limits = {}) {
    ...
    if (data.length > maxArrayElements) {
        throw new ExecutionLimitError(`xan: aggregation input limit exceeded (${maxArrayElements})`, "array_elements");
    }
    ...
    if (func === "all" || func === "any") {
        ...
        for (const row of data) {
            ...
            if (++iterations > maxIterations) {
                throw new ExecutionLimitError(`xan: aggregation iteration limit exceeded (${maxIterations})`, "iterations");
            }
    ...
    for (const row of data) {
        ...
        if (value !== null && value !== undefined) {
            if (values.length >= maxArrayElements) {
                throw new ExecutionLimitError(`xan: aggregation value limit exceeded (${maxArrayElements})`, "array_elements");
            }
            values.push(value);
        }
    }
...
export function buildAggRow(data, specs, evalOptions = {}, limits = {}) {
...
function joinAggregationValues(values, distinct, maxStringLength, limits) {
    const strings = [];
    ...
    for (const value of values) {
        ...
        const addedBytes = utf8ByteLength(stringValue) + (strings.length > 0 ? 1 : 0);
        if (addedBytes > maxStringLength - outputBytes) {
            throw new ExecutionLimitError(`xan: aggregation string limit exceeded (${maxStringLength} bytes)`, "string_length");
        }
