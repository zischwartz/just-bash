...
import { DerivedCsvBudget, formatCsv, readCsvInput, safeSetRow, toSafeRow, } from "./csv.js";
import { getMoonbladeLimits, parseMoonblade, parseNamedExpressions, } from "./moonblade-parser.js";
...
export async function cmdMap(args, ctx) {
    ...
    if (overwrite) {
        ...
        for (const spec of specs) {
            if (!headers.includes(spec.alias)) {
                if (newHeaders.length >= ctx.limits.maxArrayElements) {
                    throw new ExecutionLimitError(`xan map: output column limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
                }
        ...
        if (checkedAdd(headers.length, specs.length, "xan map") >
...
export async function cmdTransform(args, ctx) {
    ...
    const moonbladeLimits = getMoonbladeLimits(ctx);
    const ast = moonbladeToJq(parseNamedExpressions(transformExpr, moonbladeLimits)[0]?.expr ||
        parseMoonblade(transformExpr, moonbladeLimits), true, moonbladeLimits);
    const evalOptions = {
        limits: ctx.limits
            ? {
                maxIterations: ctx.limits.maxJqIterations,
