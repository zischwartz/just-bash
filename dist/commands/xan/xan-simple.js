...
import { createSafeRow, DerivedCsvBudget, formatCsv, formatCsvWithoutHeaders, parseCsv, readCsvInput, safeSetRow, toSafeRow, } from "./csv.js";
import { getMoonbladeLimits, parseNamedExpressions, } from "./moonblade-parser.js";
        ...
        for (const h of headers) {
            if (!knownHeaders.has(h)) {
                ...
                if (knownHeaders.size > ctx.limits.maxArrayElements) {
                    throw new ExecutionLimitError(`xan cat: output column limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
                }
                allHeaders.push(h);
            }
        }
...
export async function cmdFlatmap(args, ctx) {
    ...
    if (checkedAdd(headers.length, specs.length, "xan flatmap") >
        ctx.limits.maxArrayElements) {
        throw new ExecutionLimitError(`xan flatmap: output column limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
    }
    ...
    for (const row of data) {
        ...
        for (const spec of specs) {
            ...
            if (expanded.length > ctx.limits.maxArrayElements) {
                throw new ExecutionLimitError(`xan flatmap: expression result limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
            }
