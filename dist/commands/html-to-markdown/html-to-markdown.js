    ...
    async execute(args, ctx) {
        ...
        if (files.length === 0 || (files.length === 1 && files[0] === "-")) {
            if (latin1FromBytes(ctx.stdin).length > maxInputBytes) {
                throw new ExecutionLimitError(`html-to-markdown: input size limit exceeded (${maxInputBytes} bytes)`, "string_length");
            }
        ...
        for (let index = 0; index < input.length; index++) {
            if (input.charCodeAt(index) === 60 && ++tagCount > maxOperations) {
                throw new ExecutionLimitError(`html-to-markdown: complexity limit exceeded (${maxOperations} tags)`, "iterations");
            }
        }
        ...
        if (inputSize > Math.floor((maxOutputBytes - 1) / expansionFactor)) {
            throw new ExecutionLimitError(`html-to-markdown: prospective output size limit exceeded (${maxOutputBytes} bytes)`, "output_size");
        }
        const prospectiveOutputBytes = inputSize * expansionFactor + 1;
        const inputLease = ctx.executionScope?.reserveBytes("html input", inputSize, "html-to-markdown");
        const prospectiveOutputLease = ctx.executionScope?.reserveBytes("html prospective output", prospectiveOutputBytes, "html-to-markdown");
        try {
            ...
            if (outputBytes > maxOutputBytes) {
                throw new ExecutionLimitError(`html-to-markdown: output size limit exceeded (${maxOutputBytes} bytes)`, "output_size");
            }
