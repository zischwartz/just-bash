...
export const pasteCommand = {
    ...
    async execute(args, ctx) {
        ...
        if (files.length > ctx.limits.maxArrayElements) {
            throw new ExecutionLimitError(`paste: file operand limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
        }
        if (utf8ByteLength(delimiter) > ctx.limits.maxStringLength) {
            throw new ExecutionLimitError(`paste: delimiter size limit exceeded (${ctx.limits.maxStringLength} bytes)`, "string_length");
        }
        ...
        const retain = (lines) => {
            if (lines.length > ctx.limits.maxArrayElements - retainedLines) {
                throw new ExecutionLimitError(`paste: line limit exceeded (${ctx.limits.maxArrayElements})`, "array_elements");
            }
            ...
            if (file === "-") {
                ...
                try {
                    ...
                    if (contentBytes > ctx.limits.maxInputBytes - inputBytes) {
                        throw new ExecutionLimitError(`paste: aggregate input size limit exceeded (${ctx.limits.maxInputBytes} bytes)`, "string_length");
                    }
        ...
        const output = new BoundedStringBuilder(Math.min(ctx.limits.maxOutputSize, ctx.limits.maxStringLength), "paste");
        if (serial) {
