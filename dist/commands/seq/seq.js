import { boundedJoin, checkedAdd, checkedMultiply, } from "../../bounded-builder.js";
...
export const seqCommand = {
    ...
    async execute(args, ctx) {
        ...
        const maxIterations = Math.min(ctx.limits.maxLoopIterations, ctx.limits.maxArrayElements);
        ...
        const appendResult = (value) => {
            if (iterations >= maxIterations) {
                throw new ExecutionLimitError(`seq: iteration limit exceeded (${maxIterations})`, "iterations");
            }
            const nextSize = checkedAdd(checkedAdd(projectedOutputBytes, results.length > 0 ? separatorBytes : 0, "seq"), checkedAdd(utf8ByteLength(value), 1, "seq"), "seq");
            if (nextSize > maxOutputSize) {
                throw new ExecutionLimitError(`seq: output size limit exceeded (${maxOutputSize} bytes)`, "output_size");
            }
        ...
        if (increment > 0) {
            for (let n = first; n <= last + 1e-10; n += increment) {
                appendResult(precision > 0 ? n.toFixed(precision) : String(Math.round(n)));
            }
        }
        else {
            for (let n = first; n >= last - 1e-10; n += increment) {
        ...
        if (equalizeWidth && results.length > 0) {
            ...
            for (const result of results) {
                maxLen = Math.max(maxLen, result.startsWith("-") ? result.length - 1 : result.length);
            }
            const valuesBytes = results.reduce((total, result) => checkedAdd(total, maxLen + (result.startsWith("-") ? 1 : 0), "seq"), 0);
            const paddedOutputBytes = checkedAdd(checkedAdd(valuesBytes, checkedMultiply(separatorBytes, Math.max(0, results.length - 1), "seq"), "seq"), 1, "seq");
