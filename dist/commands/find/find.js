...
import { collectNewerRefs, evaluateExpressionWithPrune, evaluateForEarlyPrune, evaluateSimpleExpression, expressionHasPrune, expressionNeedsEmptyCheck, expressionNeedsStatMetadata, isSimpleExpression, } from "./matcher.js";
...
export const findCommand = {
    ...
    async execute(args, ctx) {
        ...
        if (expressionActions.some((a) => a.type === "delete"))
            depthFirst = true;
        const effects = [];
        let exitCode = 0;
        ...
        const appendStdout = (value) => {
            if (output)
                ...
                if (bytes > ctx.limits.maxOutputSize - fallbackOutputBytes) {
                    throw new ExecutionLimitError(`find: output size limit exceeded (${ctx.limits.maxOutputSize} bytes)`, "output_size");
                }
                if (value)
        ...
        for (let searchPath of searchPaths) {
            ...
            async function processNode(item) {
                ...
                const shouldReadDir = (shouldDescendIntoSubdirs || needsEmptyCheck) && !earlyPruned;
            ...
            async function findIterative() {
                ...
                if (depthFirst) {
                    ...
                    while (workCursor < workQueue.length) {
                        ...
                        const batchEnd = Math.min(workQueue.length, workCursor + FIND_BATCH_SIZE);
            ...
            for (const effect of searchResult.effects)
                effects.push(effect);
        ...
        for (const [action, paths] of batchExecPaths) {
            if (action.type !== "exec" || !ctx.exec || paths.length === 0)
                continue;
            const cmdWithFiles = [];
            for (const part of action.command) {
                if (part === "{}")
                    cmdWithFiles.push(...paths);
                else
...
function collectActions(expr) {
    if (!expr)
        return [];
    if (expr.type === "action")
        return [expr.action];
    if (expr.type === "not")
        return collectActions(expr.expr);
