...
export function parseExpressions(args, startIndex) {
    ...
    const tokens = [];
    let i = startIndex;
    const missingArgument = (predicate) => ({
        expr: null,
        pathIndex: i,
        error: `find: missing argument to \`${predicate}'\n`,
    });
    const invalidArgument = (predicate, value) => ({
        expr: null,
        pathIndex: i,
    ...
    while (i < args.length) {
        ...
        if (arg === "-name") {
            ...
            if (!/^\d+$/.test(daysStr))
                return invalidArgument(arg, mtimeArg);
            ...
            if (!/^[0-7]{1,4}$/.test(modeStr))
                return invalidArgument(arg, permArg);
...
function containsNegatedDelete(expr, negated = false) {
    if (expr.type === "action") {
        return negated && expr.action.type === "delete";
    }
    if (expr.type === "not")
        return containsNegatedDelete(expr.expr, !negated);
    if (expr.type === "and" || expr.type === "or") {
        return (containsNegatedDelete(expr.left, negated) ||
