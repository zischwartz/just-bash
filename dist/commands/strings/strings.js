 ...
 */
function extractStrings(data, options, budget, latin1String = false, signal) {
    const results = [];
    let currentLength = 0;
    let stringStart = 0;
    // Convert string to bytes if needed
    const bytes = typeof data === "string" && !latin1String
        ? new TextEncoder().encode(data)
        : data;
    if (bytes.length > budget.maxStringLength - budget.inputBytes) {
        throw new ExecutionLimitError(`strings: aggregate input size limit exceeded (${budget.maxStringLength} bytes)`, "string_length");
    }
    budget.inputBytes += bytes.length;
    const decoder = new TextDecoder();
    const readRun = (start, end) => typeof bytes === "string"
        ? bytes.slice(start, end)
        : decoder.decode(bytes.subarray(start, end));
    const appendResult = (value) => {
        if (++budget.iterations > budget.maxIterations) {
            throw new ExecutionLimitError(`strings: iteration limit exceeded (${budget.maxIterations})`, "iterations");
        }
        if (budget.resultCount >= budget.maxArrayElements) {
            throw new ExecutionLimitError(`strings: array element limit exceeded (${budget.maxArrayElements})`, "array_elements");
        }
        const prospectiveBytes = value.length + 1;
        if (prospectiveBytes > budget.maxOutputSize - budget.outputBytes) {
            throw new ExecutionLimitError(`strings: output size limit exceeded (${budget.maxOutputSize} bytes)`, "output_size");
        }
        results.push(value);
        budget.resultCount++;
    ...
    for (let i = 0; i < bytes.length; i++) {
        ...
        const byte = typeof bytes === "string" ? bytes.charCodeAt(i) & 0xff : bytes[i];
        if (isPrintable(byte)) {
            if (currentLength === 0) {
                stringStart = i;
            }
            if (currentLength >= budget.maxStringLength) {
                throw new ExecutionLimitError(`strings: string length limit exceeded (${budget.maxStringLength} bytes)`, "string_length");
            }
            currentLength++;
        }
        else {
...
export const strings = {
    ...
    execute: async (args, ctx) => {
        ...
        const budget = {
            ...
            maxOutputSize: Math.min(ctx.limits.maxOutputSize, ctx.limits.maxStringLength),
            iterations: 0,
            inputBytes: 0,
            resultCount: 0,
            outputBytes: 0,
        ...
        const stdinBytes = () => ctx.stdin;
        if (files.length === 0) {
            // Read from stdin
            const strings = extractStrings(stdinBytes(), options, budget, true, ctx.signal);
            ...
            for (const file of files) {
                ...
                const strings = extractStrings(buffer, options, budget, typeof buffer === "string", ctx.signal);
                if (strings.length > 0) {
                    output += `${strings.join("\n")}\n`;
                }
            }
