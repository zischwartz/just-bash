...
class SedParser {
    ...
    pos = 0;
    extendedRegex = false;
    constructor(scripts, extendedRegex = false, limits) {
    ...
    parse() {
        ...
        for (const script of this.scripts) {
            const lexer = new SedLexer(script, this.limits?.maxStringLength, this.limits?.maxArrayElements);
 ...
 */
export function parseMultipleScripts(scripts, extendedRegex = false, limits) {
    if (limits) {
        if (scripts.length > limits.maxArrayElements) {
            throw new ExecutionLimitError(`sed: script count limit exceeded (${limits.maxArrayElements})`, "array_elements");
        }
        ...
        for (const script of scripts) {
            ...
            if (combinedBytes > limits.maxStringLength) {
                throw new ExecutionLimitError(`sed: script size limit exceeded (${limits.maxStringLength} bytes)`, "string_length");
            }
        }
    }
    ...
    const parser = new SedParser([combinedScript], extendedRegex || extendedRegexFromComment, limits);
