...
export class SedLexer {
    ...
    line = 1;
    column = 1;
    constructor(input, maxInputLength = 1024 * 1024, maxTokens = 100_000) {
    ...
    tokenize() {
        if (this.input.length > this.maxInputLength) {
            throw new ExecutionLimitError(`sed: script size limit exceeded (${this.maxInputLength} bytes)`, "string_length");
        }
        const tokens = [];
        while (this.pos < this.input.length) {
            ...
            if (token) {
                if (tokens.length >= this.maxTokens - 1) {
                    throw new ExecutionLimitError(`sed: token limit exceeded (${this.maxTokens})`, "array_elements");
                }
                tokens.push(token);
            }
        }
