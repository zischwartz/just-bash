const NON_EXCLUDABLE_VIOLATION_TYPES = new Set([
    "function_constructor",
    "async_function_constructor",
    "generator_function_constructor",
    "async_generator_function_constructor",
]);
export function assertExcludableViolationTypes(violationTypes) {
    const nonExcludable = violationTypes?.find((type) => NON_EXCLUDABLE_VIOLATION_TYPES.has(type));
    if (nonExcludable) {
        throw new RangeError(`defenseInDepth.excludeViolationTypes cannot disable the non-excludable "${nonExcludable}" protection`);
    }
}
...
export function formatViolationErrorMessage(message, violationType, canExclude) {
