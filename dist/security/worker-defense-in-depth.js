...
const WORKER_SPECIAL_EXCLUDABLE_VIOLATION_TYPES = new Set([
...
export class WorkerSecurityViolationError extends Error {
    ...
    constructor(message, violation) {
        super(formatViolationErrorMessage(message, violation.type, WORKER_SPECIAL_EXCLUDABLE_VIOLATION_TYPES.has(violation.type) ||
...
try {
    ...
    const moduleApi = typeof getBuiltinModule === "function"
...
export class WorkerDefenseInDepth {
    ...
    applyPatches() {
        ...
        const criticalFailures = this.patchFailures.filter((path) => criticalPaths.includes(path));
        if (criticalFailures.length > 0) {
            this.restorePatches();
            throw new Error(`WorkerDefenseInDepth: critical patches failed: ${criticalFailures.join(", ")}`);
        }
    ...
    protectModuleMethod(prop, violationType) {
        ...
        try {
            ...
            if (!descriptor)
                throw new Error("method descriptor is unavailable");
