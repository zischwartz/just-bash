...
import { getBlockedGlobals, getBlockedGlobalViolationTypes, } from "./blocked-globals.js";
import { getSafeTimestamp } from "./safe-timestamp.js";
import { assertExcludableViolationTypes, formatViolationErrorMessage, } from "./violation-error-message.js";
...
let nodeModuleApi = null;
let nodeModuleClass = null;
...
export class SecurityViolationError extends Error {
    ...
    constructor(message, violation) {
        super(formatViolationErrorMessage(message, violation.type, violation.type === "error_prepare_stack_trace" ||
...
function hasContextualDynamicImportProtection() {
...
function resolveConfig(config) {
    const supplied = typeof config === "boolean" ? { enabled: config } : (config ?? {});
    ...
    if (supplied.enabled !== undefined &&
        ...
        throw new RangeError('defenseInDepth.enabled must be true, false, or "auto"');
    }
    const merged = {
        ...DEFAULT_CONFIG,
        ...supplied,
    };
    const requested = merged.enabled === "auto"
        ? "auto"
        : merged.enabled
            ? "enabled"
            : "disabled";
...
function configsEqual(left, right) {
    ...
    return (leftExcluded.length === rightExcluded.length &&
...
export class DefenseInDepthBox {
    ...
    static getInstance(config) {
        ...
        if (!DefenseInDepthBox.instance) {
            ...
            if (!configsEqual(resolved, active)) {
                throw new Error("DefenseInDepthBox config conflict: all resolved defense options, " +
    ...
    static isTrustedContext(store) {
        if (!store || store.forceUntrusted)
            return false;
        return (store.trusted === true ||
    ...
    getStatus() {
        const contextualDynamicImportProtection = hasContextualDynamicImportProtection();
        if (this.config.requested === "disabled") {
            return {
                ...
                processLifetimeIntrinsicHardening: this.config.processLifetimeIntrinsicHardening === true,
        ...
        if (!executionContext || IS_BROWSER) {
            return {
                ...
                intrinsicProtection: this.config.processLifetimeIntrinsicHardening
                    ? "process-lifetime"
                    : "scoped-best-effort",
                reason: "context-aware ESM loader hooks (node:module.registerHooks) are unavailable",
            };
        }
        return {
            requested: this.config.requested,
    ...
    updateConfig(config) {
        if (this.refCount > 0) {
            throw new Error("DefenseInDepthBox configuration cannot change while protection is active");
        }
    ...
    activate() {
        ...
        if (!IS_BROWSER && this.config.enabled && !executionContext) {
            throw new Error("DefenseInDepthBox: enabled protection requires node:async_hooks and node:module");
        }
     ...
     */
    static async runUntrustedAsync(fn) {
        if (!executionContext)
            return fn();
        const current = executionContext.getStore();
        if (!current)
    ...
    shouldAudit() {
        if (!this.config.auditMode || !executionContext)
            return false;
        const store = executionContext.getStore();
        return (store?.sandboxActive === true &&
    ...
    applyPatches() {
        ...
        const excludedViolationTypes = new Set(this.config.excludeViolationTypes ?? []);
        ...
        for (const blocked of blockedGlobals) {
            if (excludedViolationTypes.has(blocked.violationType))
                continue;
        ...
        for (const blocked of permanentIntrinsicPatches) {
            this.applyPatch(blocked, this.config.processLifetimeIntrinsicHardening === true);
        }
        if (this.config.processLifetimeIntrinsicHardening) {
     ...
     */
    protectReversibleIntrinsics(blockedGlobals) {
        const protectedTargets = new Set();
        const protect = (target, prop) => {
        ...
        for (const { target, prop } of blockedGlobals) {
            const intrinsic = target[prop];
            if (typeof intrinsic !== "object" || intrinsic === null)
                continue;
            protectedTargets.add(intrinsic);
            for (const key of Reflect.ownKeys(intrinsic))
                protect(intrinsic, key);
        }
        ...
        symbolProperties.push([Symbol.prototype, Symbol.toPrimitive], [Date.prototype, Symbol.toPrimitive], [Function.prototype, Symbol.hasInstance], [Array.prototype, Symbol.unscopables]);
        for (const sym of [
            Symbol.match,
            Symbol.matchAll,
            Symbol.replace,
        ...
        const wrapMutationMethod = (owner, prop, operation) => {
            ...
            const guarded = function (...args) {
                ...
                if (box.shouldBlock() &&
                    ...
                    const violation = box.recordViolation("prototype_mutation", path, message);
                    throw new SecurityViolationError(message, violation);
                }
    ...
    protectDynamicImport() {
        ...
        try {
            ...
            const shouldRecordAuditViolation = (box) => {
                const store = executionContext?.getStore();
            ...
            throw new Error("context-aware ESM loader hooks (node:module.registerHooks) are unavailable");
        }
        catch (e) {
            throw new Error(`DefenseInDepthBox: could not register dynamic-import hooks: ${e instanceof Error ? e.message : String(e)}`, { cause: e });
        }
    }
    /**
    ...
    protectModuleMethod(prop, violationType) {
        ...
        try {
            ...
            const proxy = this.createBlockingProxy(original, path, violationType);
            this.originalDescriptors.push({
                target: ModuleClass,
                prop,
                descriptor,
     ...
     */
    applyPatch(blocked, freezeIntrinsic = false) {
        ...
        try {
            ...
            if (strategy === "freeze") {
                ...
                if (typeof original === "object" && original !== null) {
                    ...
                    const proxy = this.createReadonlyObjectProxy(original, path, violationType);
    ...
    restorePatches() {
        for (let i = this.temporaryIntrinsicDescriptors.length - 1; i >= 0; i--) {
            const { target, prop, descriptor } = this.temporaryIntrinsicDescriptors[i];
            try {
                Object.defineProperty(target, prop, descriptor);
            }
            catch (e) {
                console.debug(`[DefenseInDepthBox] Could not restore temporary intrinsic ${String(prop)}:`, e instanceof Error ? e.message : e);
            }
        }
