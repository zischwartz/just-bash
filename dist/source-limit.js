...
export function assertSourceWithinLimit(source, maxSourceBytes) {
    if (source.length > maxSourceBytes ||
        utf8ByteLength(source) > maxSourceBytes) {
        throw new ExecutionLimitError(`script input size limit exceeded (${maxSourceBytes} bytes)`, "string_length");
    }
}
